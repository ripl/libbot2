...
# Converts a LCM log to a "matrix" format that is easier to work with in
# external tools such as Matlab. The set of messages on a given channel
# can be represented as a matrix, where the columns of this matrix are
# the the fields of the lcm type with one message per row
...
-s --seperator=sep             Print data with separator [sep] instead of
                               default to ["" ""]
-c --channelsToProcess=chan    Parse channelsToProcess that match Python regex
                               [chan] defaults to [".*"]
-i --ignore=chan               Ignore channelsToProcess that match Python regex
                               [chan] ignores take precedence over includes!
-o --outfile=ofname            Output data to [ofname] instead of default
                               [filename.mat or stdout]
-l --lcmtype_pkgs=pkgs         Load Python modules from comma seperated list of
                               packages [pkgs] defaults to ["botlcm"]
-v                             Verbose
""")
    sys.exit()
...
def make_numpy_array_accessor(fieldname):
    return lambda lst, x: lst.extend(
        numpy.array(getattr(x, fieldname)).ravel())
...
def make_lcmtype_accessor(msg):
    ...
    for fieldname in getattr(msg, '__slots__'):
        ...
        if isinstance(m, (int, float, bool)):
            ...
            funcs.append(make_obj_accessor(fieldname,
                                           make_lcmtype_accessor(m)))
    ...
    for fieldname in getattr(msg, '__slots__'):
        ...
        if isinstance(m, (int, float, bool)):
            ...
            if arr.dtype.kind in "bif":
                ...
                if base:
                    typeStr.append("%d- %s(%d)" %
                                   (count + 1, fieldname, len(arr.ravel())))
                else:
                ...
                numSub = len(m)
                if base:
                    subStr = "%d- %s<%s>(%d)" % (count + 1, fieldname,
                                                 ", ".join(subStr), numSub)
                else:
                    subStr = "%s<%s>(%d)" % (fieldname, ", ".join(subStr),
                                             numSub)
...
longOpts = [
    "help", "print", "format", "separator", "channelsToProcess", "ignore",
    "outfile", "lcm_packages"
]
...
for e in log:
    ...
    if e.channel in ignored_channels:
        continue
    if ((checkIgnore and channelsToIgnore.match(e.channel)
    ...
    if not (arr.dtype.kind in "bif"):
        statusMsg = deleteStatusMsg(statusMsg)
        sys.stderr.write(
            "WARNING: needed to create new flattener for channel %s\n" %
            (e.channel))
        flattener = make_flattener(msg)
        flatteners[e.channel] = flattener
        a = flattener(msg)
...
if not printOutput:
    ...
    for chan in data:
        ...
        minLen = min(lengths)
        if maxLen != minLen:
            sys.stderr.write("padding channel "
                             "%s with zeros, messages ranged from %d to %d\n" %
                             (chan, minLen, maxLen))
            count = 0
            for i in data[chan]:
