...
static sender_t* sender_new(struct in_addr addr, uint16_t port) {
...
static void sender_destroy(sender_t* sender) {
  free(sender->id_str);
...
static host_t* host_new(struct in_addr addr) {
...
static sender_t* host_get_sender(host_t* host, uint16_t port) {
  int kv = (int)port;
  return g_hash_table_lookup(host->senders, &kv);
}

static sender_t* host_add_new_sender(host_t* host, uint16_t port) {
...
static int lcm_parse_url(const char* url, struct in_addr* mc_addr,
                         uint16_t* mc_port) {
  if (!url || !strlen(url)) {
    return -1;
  }
...
static int setup_socket(state_t* app) {
  ...
  int opt = 1;
  if (setsockopt(app->recvfd, SOL_SOCKET, SO_REUSEADDR, (char*)&opt,
                 sizeof(opt)) < 0) {
  ...
  if (setsockopt(app->recvfd, SOL_SOCKET, SO_REUSEPORT, (char*)&opt,
                 sizeof(opt)) < 0) {
  ...
  if (setsockopt(app->recvfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, (char*)&mreq,
                 sizeof(mreq)) < 0) {
...
static int on_message_ready(GIOChannel* source, GIOCondition cond,
                            void* user_data) {
  ...
  int sz =
      recvfrom(app->recvfd, buf, 65536, 0, (struct sockaddr*)&from, &fromlen);
...
static gboolean on_timer(void* user_data) {
  state_t* app = (state_t*)user_data;
...
static const char* get_default_lcm_url(void) {
...
int main(int argc, char** argv) {
  ...
  app->sid = g_io_add_watch(app->ioc, G_IO_IN, (GIOFunc)on_message_ready, app);
