...
static inline int64_t packet_sig(const char* channel) {
...
static void on_lcm_tunnel_introspect(const lcm_recv_buf_t* rbuf,
  ...
  if (!data_offset_okay || rbuf->data != lbuf->buf + doff ||
...
void introspect_send_introspection_packet(introspect_t* self) {
...
void introspect_destroy(introspect_t* self) {
  lcm_unsubscribe(self->lcm, self->subscription);
  self->subscription = NULL;
  free(self);
}

int introspect_is_message_from_self(introspect_t* self,
                                    const lcm_recv_buf_t* rbuf,
                                    const char* channel) {
  return packet_sig(channel) == self->sig;
}

int introspect_is_ready(const introspect_t* self) { return self->ready; }
