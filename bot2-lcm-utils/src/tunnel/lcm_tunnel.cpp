...
static inline void check_ret(int ret) { assert(ret == 0); }
...
LcmTunnel::LcmTunnel(bool verbose, const char* lcm_channel)
    : verbose(verbose),
      ...
      bytesInQueue(0),
      channel((char*)calloc(65536, sizeof(char))),
      channel_sz(65536),
      ...
      server_udp_port(-1),
      udp_send_seqno(0),
      recFlags((char*)calloc(1024, sizeof(char))),
      recFlags_sz(1024),
      cur_seqno(0),
      errorStartTime(-1),
...
void LcmTunnel::init_regex(const char* lcm_channel) {
  if (lcm_channel && strlen(lcm_channel)) {
    ...
    GError* rerr = NULL;
    regex = g_regex_new(rchannel, (GRegexCompileFlags)0, (GRegexMatchFlags)0,
                        &rerr);
    if (rerr != NULL) {
...
bool LcmTunnel::match_regex(const char* lcm_channel) {
  return regex != NULL &&
      (g_regex_match(regex, lcm_channel, (GRegexMatchFlags)0, NULL));
}
...
int LcmTunnel::connectToClient(
    lcm_t* lcm_, introspect_t* introspect_, GMainLoop* mainloop_,
    ssocket_t* sock_,
    tunnel_server_params_t*
        server_params_) {  // for a client that connected to this server
  ...
  snprintf(name, sizeof(name), "%s:%d", inet_ntoa(client_addr.sin_addr),
           client_port);
...
int LcmTunnel::connectToServer(
    lcm_t* lcm_, introspect_t* introspect_, GMainLoop* mainloop_,
    char* server_addr_str, int port, char* channels_to_recv,
    lcm_tunnel_params_t* tunnel_params_,
    tunnel_server_params_t*
        server_params_) {  // for a client that should initiate a connection
                           // with a server
  ...
  snprintf(name, sizeof(name), "%s:%d", inet_ntoa(server_addr.sin_addr),
           server_port);
  fprintf(stderr, "Connected to %s\n", name);

  // transmit subscription information
  lcm_tunnel_params_t* tun_params_to_send =
      lcm_tunnel_params_t_copy(tunnel_params);
  ...
  if (tunnel_params->udp) {
    ...
    subscription =
        lcm_subscribe(lcm, tunnel_params->channels, on_lcm_message, this);
...
void LcmTunnel::publishLcmMessagesInBuf(int numBytes) {
  ...
  while (msgOffset < numBytes) {
    ...
    msgOffset +=
        lcm_tunnel_sub_msg_t_decode(buf, msgOffset, numBytes - msgOffset, &p);
    // and publish
    LcmTunnelServer::check_and_send_to_tunnels(p.channel, p.data, p.data_size,
                                               this);
...
int LcmTunnel::on_udp_data(GIOChannel* source, GIOCondition cond,
                           void* user_data) {
  ...
  lcm_tunnel_udp_msg_t* recv_udp_msg =
      (lcm_tunnel_udp_msg_t*)calloc(1, sizeof(lcm_tunnel_udp_msg_t));
  int decode_ret =
      lcm_tunnel_udp_msg_t_decode(recv_buffer, 0, recv_status, recv_udp_msg);
  if (decode_ret < 0) {
    lcm_tunnel_disconnect_msg_t disc_msg;
    decode_ret = lcm_tunnel_disconnect_msg_t_decode(recv_buffer, 0, recv_status,
                                                    &disc_msg);
    if (decode_ret >= 0) {
  ...
  if (recv_udp_msg->seqno > self->cur_seqno ||
      recv_udp_msg->seqno < (int32_t)self->cur_seqno -
              SEQNO_WRAP_GAP) {  // handle wrap-around with second part
    if ((!self->message_complete && self->cur_seqno > 0) ||
        recv_udp_msg->seqno > (self->cur_seqno + 1)) {
      printf("packets %d to %d dropped! with %d of %d fragments received, ",
             self->cur_seqno, recv_udp_msg->seqno - 1, self->numFragsRec,
             self->nfrags);
      if (self->tunnel_params->fec > 1 &&
          self->nfrags >= MIN_NUM_FRAGMENTS_FOR_FEC) {
        printf("was FECed\n");
      } else {
        printf("not FECed\n");
      }
    ...
    memset(self->recFlags, 0,
           self->recFlags_sz);  // mark all frags as unreceived
    self->completeTo_fragno = 0;
    self->fragment_buf_offset = 0;

    int messageSize = recv_udp_msg->payload_size;
    // increase buffer size if needed, also make enough space for the channel in
    // case we're using FEC
    ...
    if (self->tunnel_params->fec > 1 &&
      ...
      self->ldpc_dec =
          new ldpc_dec_wrapper(messageSize, MAX_PAYLOAD_BYTES_PER_FRAGMENT,
                               self->tunnel_params->fec);
    }
    self->message_complete = 0;
  }

  if (!self->message_complete && recv_udp_msg->seqno == self->cur_seqno &&
      getNumFragments(recv_udp_msg->payload_size) == self->nfrags) {
    self->numFragsRec++;
    if (self->tunnel_params->fec < 1 ||
        self->nfrags < MIN_NUM_FRAGMENTS_FOR_FEC) {  // we're not using FEC for
                                                     // this message
      // have we already received this fragment?
      if (recv_udp_msg->fragno < self->nfrags &&
          !self->recFlags[recv_udp_msg->fragno]) {
        self->recFlags[recv_udp_msg->fragno] = 1;

        // copy everything to the app->buf
        int64_t pos_start =
            recv_udp_msg->fragno * MAX_PAYLOAD_BYTES_PER_FRAGMENT;
        int64_t pos_end =
            MIN(recv_udp_msg->payload_size,
                (recv_udp_msg->fragno + 1) * MAX_PAYLOAD_BYTES_PER_FRAGMENT);
      ...
      int dec_done = self->ldpc_dec->processPacket(recv_udp_msg->data,
                                                   recv_udp_msg->fragno);
      if (dec_done != 0) {
        if (dec_done == 1) {
          ...
          fprintf(stderr,
                  "ldpc got all the sent packets, but couldn't reconstruct... "
    ...
    printf(
        "ignoring udp packet seqno=%d, nfrag =%d, \t self-> seqno=%d, "
        "nfrags=%d\n",
        recv_udp_msg->seqno, getNumFragments(recv_udp_msg->payload_size),
...
int LcmTunnel::on_tcp_data(GIOChannel* source, GIOCondition cond,
                           void* user_data) {
  int ret = TRUE;
  LcmTunnel* self = (LcmTunnel*)user_data;
  ...
  ssize_t nread =
      read(ssocket_get_fd(self->tcp_sock), self->buf + self->bytes_read,
  ...
  switch (self->tunnel_state) {
    ...
    case CLIENT_MSG_DATA: {
      ...
      int decode_status =
          lcm_tunnel_params_t_decode(self->buf, 0, self->bytes_read, &tp_rec);
      if (decode_status <= 0) {
      ...
      if (self->tunnel_params->udp) {
        ...
        struct sockaddr_in client_addr;
        socklen_t addrlen = sizeof(client_addr);
        getpeername(self->tcp_sock->socket, (struct sockaddr*)&client_addr,
                    &addrlen);
        ...
        connect(self->udp_fd, (struct sockaddr*)&client_addr,
                sizeof(client_addr));
        ...
        uint32_t msg_sz_n = htonl(msg_sz);
        if (4 !=
            _fileutils_write_fully(ssocket_get_fd(self->tcp_sock), &msg_sz_n,
                                   4)) {
          perror("sending subscription data");
          LcmTunnelServer::disconnectClient(self);
          return FALSE;
        ...
        if (msg_sz !=
            _fileutils_write_fully(ssocket_get_fd(self->tcp_sock), msg,
                                   msg_sz)) {
          perror("sending subscription data");
          LcmTunnelServer::disconnectClient(self);
          return FALSE;
        ...
        self->udp_sid = g_io_add_watch(self->udp_ioc, G_IO_IN,
                                       LcmTunnel::on_udp_data, self);
      ...
      fprintf(stderr, "%s subscribed to \"%s\" -- ", self->name,
              self->tunnel_params->channels);

      if (self->udp_fd >= 0) {
        if (self->tunnel_params->fec > 1) {
          fprintf(stderr, "UDP with FEC rate of %.2f and max_delay of %dms\n",
                  self->tunnel_params->fec, self->tunnel_params->max_delay_ms);
      ...
      self->subscription = lcm_subscribe(
          self->lcm, self->tunnel_params->channels, on_lcm_message, self);
    ...
    case RECV_DATA:
      ...
      LcmTunnelServer::check_and_send_to_tunnels(self->channel, self->buf,
                                                 self->bytes_read, self);
      lcm_publish(self->lcm, self->channel, (uint8_t*)self->buf,
                  self->bytes_read);
...
gpointer LcmTunnel::sendThreadFunc(gpointer user_data) {
  ...
  while (!self->stopSendThread) {
    if (self->sendQueue.empty()) {
      ...
      nextFlushTime =
          _timestamp_now() + self->tunnel_params->max_delay_ms * 1000;
      continue;
    }
    int64_t now = _timestamp_now();
    if (self->tunnel_params->max_delay_ms > 0 &&
        self->bytesInQueue < NUM_BYTES_TO_SEND_IMMEDIATELY &&
        nextFlushTime > now && !self->flushImmediately) {
      g_cond_wait_until(self->sendQueueCond, self->sendQueueLock,
                        nextFlushTime);
...
void LcmTunnel::send_to_remote(const void* data, uint32_t len,
                               const char* lcm_channel) {
  lcm_recv_buf_t rbuf;
  rbuf.data = (void*)data;
  rbuf.data_size = len;
  rbuf.recv_utime = _timestamp_now();  // use current timestamp, should be close
                                       // to when received...
  rbuf.lcm = this->lcm;
  send_to_remote(&rbuf, lcm_channel);
}

void LcmTunnel::send_to_remote(const lcm_recv_buf_t* rbuf,
                               const char* lcm_channel) {
  g_mutex_lock(sendQueueLock);
  ...
  while (bytesInQueue > MAX_SEND_BUFFER_SIZE) {
    fprintf(stderr,
            "Warning: send queue is too big (%dMB), dropping messages\n",
            bytesInQueue / (2 << 20));
...
void LcmTunnel::on_lcm_message(const lcm_recv_buf_t* rbuf, const char* channel,
                               void* user_data) {
  LcmTunnel* self = (LcmTunnel*)user_data;
...
void LcmTunnel::checkUDPSendStatus(int send_status) {
  ...
  if (send_status < 0) {
    ...
    if (numSuccessful > 3) {
      fprintf(stderr,
              "Connection may be back up after %fsec send_status=%d "
              "numSuccessful=%d\n",
              (double)(now - errorStartTime) * 1e-6, send_status,
              numSuccessful);
      perror("perror: ");
      lastErrorPrintTime = -1;
      errorStartTime = -1;
...
bool LcmTunnel::send_lcm_messages(std::deque<TunnelLcmMessage*>& msgQueue,
                                  uint32_t bytesInQueue) {
  if (udp_fd >= 0) {
    if (server_udp_port <= 0) {  // connection hasn't been setup yet.
      return true;
    }
    ...
    if (verbose) {
      printf("sending %d bytes from %d lcm messages\n", bytesInQueue,
             static_cast<int>(msgQueue.size()));
    }

    uint32_t msgSize = bytesInQueue;
    int nfragments = getNumFragments(msgSize);
    if ((tunnel_params->fec <= 0 && nfragments > MAX_NUM_FRAGMENTS) ||
        (tunnel_params->fec > 0 &&
         nfragments > MAX_NUM_FRAGMENTS / tunnel_params->fec)) {
      uint32_t maxMsgSize;
      ...
      fprintf(stderr,
              "WARNING! Queue contains more than the max message size of %d "
              "bytes... we're WAY behind, dropping msgs\n",
              maxMsgSize);
    ...
    while (!msgQueue.empty()) {
      ...
      msgQueue.pop_front();
      lcm_tunnel_sub_msg_t_encode(msgBuf, msgBufOffset, msgSize - msgBufOffset,
                                  msg->sub_msg);
    ...
    if (tunnel_params->fec < 1 ||
        nfragments < MIN_NUM_FRAGMENTS_FOR_FEC) {  // don't use FEC
      int sendRepeats = 1;
      if (fabs(tunnel_params->fec) > 1) {  // fec <0 means always send
                                           // duplicates
        sendRepeats = (int)ceil(
            fabs(tunnel_params->fec));  // send ceil of the fec rate times
      }
      for (int r = 0; r < sendRepeats; r++) {
        ...
        for (int i = 0; i < nfragments; i++) {
          ...
          msg.data_size =
              MIN(MAX_PAYLOAD_BYTES_PER_FRAGMENT, msgSize - msgBufOffset);
      ...
      ldpc_enc_wrapper* ldpc_enc = new ldpc_enc_wrapper(
          msgBuf, msgSize, MAX_PAYLOAD_BYTES_PER_FRAGMENT, tunnel_params->fec);
    ...
    while (!msgQueue.empty()) {
      ...
      if (tunnel_params->tcp_max_age_ms > 0 &&
          age_ms > tunnel_params->tcp_max_age_ms) {
        // message has been queued up for too long.  Drop it.
        if (verbose) {
          fprintf(stderr,
                  "%s message too old (age = %d, param = %d), dropping.\n",
                  msg->sub_msg->channel, (int)age_ms,
                  tunnel_params->tcp_max_age_ms);
        }
      } else {
        // send channel
        ...
        if (chan_len !=
            _fileutils_write_fully(cfd, msg->sub_msg->channel, chan_len)) {
          delete msg;
          return false;
        }
        ...
        if (msg->sub_msg->data_size !=
            _fileutils_write_fully(cfd, msg->sub_msg->data,
                                   msg->sub_msg->data_size)) {
          delete msg;
          return false;
        }
      }
...
static void usage(const char* progname) {
  ...
  printf(
      "Usage: %s [options] <server_addr[:server_port]>\n"
      ...
      "                              these channels will be forwarded to the "
      "server\n"
      "                              is automatically surrounded by ^ and $.\n"
      "                              (Default: .*)\n"
      ...
      "    -p, --port=N              Start server on control port N instead "
      "of\n"
      "                              default (%d)\n"
      "\n"
      "    -u, --udp                 Request server transmit via UDP instead "
      "of TCP\n"
      "\n"
      "    -m, --tcp-max-age-ms=AGE  Instructs the server not to tunnel "
      "messages\n"
      "                              that have been queued up and waiting for\n"
      "                              delivery for more than AGE ms.  If less "
      "than\n"
      "                              or equal to zero, then messages can be "
      "queued\n"
      "                              indefinitely.  This option is not used "
      "when -u\n"
      "                              is specified.  Default: 10000\n"
      "\n"
      "    -f, --fec=FEC             Request server to use UDP packets with "
      "Forward\n"
      "                              Error Correction applied at a rate of FEC "
      "\n"
      "                              (must be >1) small messages will be "
      "duplicated \n"
      "                              ceil(FEC) times instead of coding.       "
      "\n"
      "\n"
      "    -d, --dup=NDUP            Request server to use UDP packets with "
      "each\n"
      "                              packet sent NDUP times for drop "
      "resiliency\n"
      "                              --fec and --dup cannot be used together\n"
      "\n"
      "\n"
      "    -w, --wait-time-ms=TIME   Request server to queue up lcm messages "
      "for\n"
      ...
      "\n"
      " %s \n"
      "    Start a server. Clients can then connect using one of the examples "
      "below.\n"
      ...
      "\n"
      " %s -s ABC 192.168.1.1\n"
      "    Server at 192.168.1.1 forwards traffic on channel ABC, we forward "
      "all\n"
      "    channels back.\n"
      "\n"
      " %s -r ABC 192.168.1.1\n"
      "    Server at 192.168.1.1 forwards all channels, we forward traffic on "
      "channel\n"
      "    ABC back.\n"
      "\n"
      " %s -u -f 1.5 -s \"ABC|DEF\" -r \"\" 192.168.1.1\n"
      "    We forward traffic on channels ABC and DEF to 192.168.1.1 via UDP "
      "with\n"
      "    FEC 1.5.  Server does not forward anything back.\n"
      "\n",
      basename, DEFAULT_PORT, DEFAULT_PORT, basename, basename, basename,
      basename, basename);
  free(basename);
  exit(1);
}
...
int main(int argc, char** argv) {
  ...
  while ((c = getopt_long(argc, argv, optstring, long_opts, 0)) >= 0) {
    switch (c) {
      ...
      case 'd': {
        ...
        params.udp = 1;  // use udp to send FEC data
        params.fec = -round(
            params.fec);  // if fec is negative, its handled as duplicates
        break;
      }
  ...
  if (params.connectToServer) {
    ...
    int ret = tunnelClient->connectToServer(
        LcmTunnelServer::lcm, LcmTunnelServer::introspect,
        LcmTunnelServer::mainloop, params.server_addr_str, params.server_port,
        params.channels_recv, &tunnel_params, &LcmTunnelServer::params);
