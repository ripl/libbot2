...
#ifndef BOT2_LCM_UTILS_TUNNEL_LCM_TUNNEL_H_
#define BOT2_LCM_UTILS_TUNNEL_LCM_TUNNEL_H_
...
// must be a multiple of 16 for LDPC, setting it a bit smaller to mitigate
// wireless interference a bit
...
#define MAX_NUM_FRAGMENTS 32768
// since we're using a int16_t for the fragment number
...
class TunnelLcmMessage {
 public:
  ...
  ~TunnelLcmMessage() { lcm_tunnel_sub_msg_t_destroy(sub_msg); }
...
class LcmTunnel {
 public:
  LcmTunnel(bool verbose,
            const char* channel);  // just allocates buffers and whatnot
  int connectToClient(lcm_t* lcm_, introspect_t* introspect_,
                      GMainLoop* mainloop_, ssocket_t* sock_,
                      tunnel_server_params_t* server_params_);
  int connectToServer(lcm_t* lcm_, introspect_t* introspect_,
                      GMainLoop* mainloop_, char* server_addr_str, int port,
                      char* channels_to_recv,
  ...
  static int on_tcp_data(GIOChannel* source, GIOCondition cond,
                         void* user_data);
  static int on_udp_data(GIOChannel* source, GIOCondition cond,
                         void* user_data);
...
#endif  // BOT2_LCM_UTILS_TUNNEL_LCM_TUNNEL_H_
