...
class LcmTunnelServer {
 public:
  ...
  static int acceptClient(GIOChannel* source, GIOCondition cond,
                          void* user_data);
  ...
  static void check_and_send_to_tunnels(const char* channel, const void* data,
                                        unsigned int len, LcmTunnel* to_skip);
