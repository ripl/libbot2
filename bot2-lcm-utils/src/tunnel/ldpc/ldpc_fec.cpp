...
LDPCFecSession::~LDPCFecSession() { EndSession(); }

// InitSession : Initializes the LDPC session.
// => See header file for more informations.
ldpc_error_status LDPCFecSession::InitSession(int nbSourceSymbols,
                                              int nbParitySymbols,
                                              int symbolSize, int flags,
                                              int seed, SessionType codecType,
                                              int leftDegree) {
  mod2entry* e;
  ...
  if (nbSourceSymbols + nbParitySymbols > this->GetMaxN()) {
    fprintf(stderr,
            "LDPCFecSession::InitSession: ERROR: the total number of symbols "
            "(%d) is too large (%d max)\n",
            nbSourceSymbols + nbParitySymbols, this->GetMaxN());
#ifdef SPARSE_MATRIX_OPT_SMALL_INDEX
    fprintf(stderr,
            "NB: this codec has been compiled in SPARSE_MATRIX_OPT_SMALL_INDEX "
            "mode. You can #undef it in src/ldpc_profile.h file to remove this "
            "limitation.\n");
#endif
    return LDPC_ERROR;
  }
  ...
  if ((m_sessionType != TypeLDGM) && (leftDegree != 3)) {
    fprintf(stderr,
            "LDPCFecSession::InitSession: ERROR: setting a leftDegree!=3 (here "
            "%d) is only meaningfull with LDGM codes, not with "
  ...
  if (this->m_verbosity >= 1) {
    if (m_sessionType == TypeLDGM) {
      printf(
          "Initializing LDPC FEC Session...\n - Source symbols = %d\n - Parity "
          "symbols = %d\n - Symbol size = %d\n - Edges per source symbols = "
          "%d\n",
          m_nbSourceSymbols, m_nbCheck, m_symbolSize,
          m_leftDegree = leftDegree);
    } else if (m_sessionType == TypeSTAIRS) {
      printf(
          "Initializing LDPC STAIRCASE FEC Session...\n - Source symbols = "
          "%d\n - Parity symbols = %d\n - Symbol size = %d\n - Edges per "
          "source symbols = %d\n",
          m_nbSourceSymbols, m_nbCheck, m_symbolSize,
          m_leftDegree = leftDegree);
      ...
      printf(
          "Initializing LDPC TRIANGLE FEC Session...\n - Source symbols = %d\n "
          "- Parity symbols = %d\n - Symbol size = %d\n - Edges per source "
          "symbols = %d\n",
          m_nbSourceSymbols, m_nbCheck, m_symbolSize,
          m_leftDegree = leftDegree);
  ...
  m_pchkMatrix = CreatePchkMatrix(
      m_nbCheck, m_nbSourceSymbols + m_nbParitySymbols, Evenboth, m_leftDegree,
      seed, false, m_sessionType, this->m_verbosity);
  if (m_pchkMatrix == NULL) {
    fprintf(stderr,
            "LDPCFecSession::InitSession: ERROR: call to CreatePchkMatrix "
            "failed!\n");
    return LDPC_ERROR;
  }
  if (this->m_verbosity >= 1) {
  ...
  if (m_sessionFlags & FLAG_CODER) {
    ...
    if (m_nb_unknown_symbols_encoder == NULL) {
      fprintf(stderr,
              "LDPCFecSession::InitSession: ERROR: call to calloc failed for "
              "m_nb_unknown_symbols_encoder!\n");
      return LDPC_ERROR;
    }
  ...
  if (m_sessionFlags & FLAG_DECODER) {
    ...
    if (((m_checkValues = (void**)calloc(m_nbCheck, sizeof(void*))) == NULL) ||
        ...
        ((m_nb_unknown_symbols = (int*)calloc(m_nbCheck, sizeof(int))) ==
         NULL) ||
        ((m_nbEqu_for_parity = (int*)calloc(m_nbCheck, sizeof(int))) == NULL) ||
        ((m_parity_symbol_canvas = (void**)calloc(m_nbCheck, sizeof(void*))) ==
         NULL)) {
      fprintf(stderr,
              "LDPCFecSession::InitSession: ERROR: call to calloc failed for "
              "m_parity_symbol_canvas!\n");
      return LDPC_ERROR;
    }
    ...
    for (int seq = m_nbSourceSymbols; seq < m_nbTotalSymbols; seq++) {
      for (e = mod2sparse_first_in_col(m_pchkMatrix, GetMatrixCol(seq));
  ...
  m_triangleWithSmallFECRatio = (m_sessionType == TypeTRIANGLE) &&
      ((m_nbTotalSymbols / m_nbSourceSymbols) < 2.0);
#ifdef DEBUG
...
ldpc_error_status LDPCFecSession::SetCallbackFunctions(
  ...
  if (DecodedSymbol_callback) {
    if (!(m_sessionFlags & FLAG_DECODER)) {
      fprintf(stderr,
              "LDPCFecSession::SetCallbackFunctions: ERROR: specifying "
              "DecodedSymbol_callback is only valid in DECODER mode\n");
      return LDPC_ERROR;
    }
  }
  if (AllocTmpBuffer_callback) {
    if (!(m_sessionFlags & FLAG_DECODER)) {
      fprintf(stderr,
              "LDPCFecSession::SetCallbackFunctions: ERROR: specifying "
              "AllocTmpBuffer_callback is only valid in DECODER mode\n");
      return LDPC_ERROR;
    }
  }
  if (FreeSymbol_callback) {
    if (!(m_sessionFlags & FLAG_DECODER)) {
      fprintf(stderr,
              "LDPCFecSession::SetCallbackFunctions: ERROR: specifying "
              "FreeSymbol_callback is only valid in DECODER mode\n");
      return LDPC_ERROR;
    }
  }
      ...
      for (int i = 0; i < m_nbCheck; i++) {
        if (m_checkValues[i] != NULL) {
          ...
          if (m_freeSymbol_callback != NULL) {
            m_freeSymbol_callback(m_context_4_callback, m_checkValues[i]);
          } else {
#endif
...
// => See header file for more informations.
void LDPCFecSession::SetVerbosity(int verb) { this->m_verbosity = verb; }
...
void LDPCFecSession::MoreAbout(FILE* out) {
  fprintf(
      out,
      "LDPC/LDGM large block FEC codec - Version 2.1, September 7th, 2006\n");
...
// => See header file for more informations.
void LDPCFecSession::AddToSymbol(void* to, void* from) {
  unsigned int i;
#if defined(__LP64__) || (__WORDSIZE == 64)
  // 64-bit machines
...
// => See header file for more informations.
ldpc_error_status LDPCFecSession::BuildParitySymbol(void* symbol_canvas[],
  ...
  ASSERT(m_sessionType == TypeSTAIRS || m_sessionType == TypeTRIANGLE ||
  ...
  while (!mod2sparse_at_end(e)) {
    ...
    if (e->col != paritySymbol_index) {
      ...
      to_add_buf = (uintptr_t*)GetBuffer(symbol_canvas[seqno]);
      if (to_add_buf == NULL) {
        fprintf(stderr,
                "LDPCFecSession::BuildParitySymbol: FATAL ERROR, symbol %d is "
                "not allocated!\n",
                seqno);
        return LDPC_ERROR;
      }
...
// BuildParitySymbol: Builds a new parity symbol.
// => See header file for more informations.
ldpc_error_status LDPCFecSession::BuildParitySymbolsPerCol(
    void* symbol_canvas[], int symbol_index, int* built_parity_symbols[],
  ...
  *nb_built_parity_symbols = 0;
    ...
    if (e->row != GetMatrixCol(symbol_index)) {
      ...
      if (symbol_canvas[GetSymbolSeqno(e->row)] == NULL) {
        ...
        if (m_allocTmpBuffer_callback) {
          symbol_canvas[GetSymbolSeqno(e->row)] =
              m_allocTmpBuffer_callback(m_context_4_callback, m_symbolSize);
        } else {
#endif
          symbol_canvas[GetSymbolSeqno(e->row)] =
              (char*)calloc(m_symbolSize, 1);
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
        }
#endif
      }
      paritySymbol =
          (uintptr_t*)GetBufferPtrOnly(symbol_canvas[GetSymbolSeqno(e->row)]);
      ...
      if (m_nb_unknown_symbols_encoder[e->row] == 1) {
        ...
        if (*nb_built_parity_symbols == 1) {
          *built_parity_symbols = (int*)calloc(1, sizeof(int));
        } else {
          *built_parity_symbols =
              (int*)realloc((void*)*built_parity_symbols,
                            (*nb_built_parity_symbols) * sizeof(int));
        }
  ...
  for (int i = 0; i < temp; i++) {
    ...
    BuildParitySymbolsPerCol(
        symbol_canvas, *built_parity_symbols[i] + m_nbSourceSymbols,
        &recursive_built_parity_symbols, &recursive_nb_built_parity_symbols);
    *built_parity_symbols = (int*)realloc(
        (void*)*built_parity_symbols,
        ((*nb_built_parity_symbols) + recursive_nb_built_parity_symbols) *
            sizeof(int));
...
// GetMatrixCol:
// => See header file for more informations.
int LDPCFecSession::GetMatrixCol(int symbolSeqno) {
...
// GetSymbolSeqno:
// => See header file for more informations.
int LDPCFecSession::GetSymbolSeqno(int matrixCol) {
  int colInOrder;
...
// => See header file for more informations.
bool LDPCFecSession::SymbolAlreadyKnown(void* symbol_canvas[],
                                        int new_symbol_seqno) {
  if ((mod2sparse_last_in_col(m_pchkMatrix, GetMatrixCol(new_symbol_seqno))
           ->row < 0) ||
      (IsSourceSymbol(new_symbol_seqno) &&
       (symbol_canvas[new_symbol_seqno] != NULL)) ||
      (IsParitySymbol(new_symbol_seqno) &&
    ...
    if (this->m_verbosity >= 1) {
      printf(
          "LDPCFecSession::SymbolAlreadyKnown: %s symbol %d already received "
          "or rebuilt\n",
  ...
  if (this->m_verbosity >= 1) {
    printf(
        "LDPCFecSession::SymbolAlreadyKnown: %s symbol %d not received or "
        "rebuilt\n",
        (new_symbol_seqno < m_nbSourceSymbols) ? "Source" : "Parity",
        new_symbol_seqno);
  }
...
bool LDPCFecSession::IsDecodingComplete(void* symbol_canvas[]) {
  if (!m_initialized) {
    fprintf(stderr,
            "LDPCFecSession::IsDecodingComplete: ERROR: LDPC Session is NOT "
            "initialized!\n");
    return false;
  }
