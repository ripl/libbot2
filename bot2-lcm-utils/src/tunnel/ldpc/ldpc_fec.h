 ...
 */
enum ldpc_error_status { LDPC_OK = 0, LDPC_ERROR = 1 };
...
class LDPCFecSession {
 public:
   ...
   */
  ldpc_error_status InitSession(int nbSourceSymbols, int nbParitySymbols,
                                int symbolSize, int flags = FLAG_BOTH,
                                int seed = 1,
   ...
   *   is decoded by the DecodingStepWithSymbol() function. What this function
   *   does is application-dependant, but it must return a pointer to a data
   *   buffer, left uninitialized, of the appropriate size.
   ...
   * - The FreeSymbol callback is called each time a symbol (or temporary
   *   buffer) is no longer required and can be free'd by the memory mgmt
   *   system.
   *
   ...
   * @param paritySymbol_index  (IN)  Index of parity symbol to build in
   *        {0.. n-k-1} range (!)
   ...
   * @return      Completion status (LDPC_OK or LDPC_ERROR).
   */
  ldpc_error_status BuildParitySymbolsPerCol(
      void* symbol_canvas[], int symbol_index,
      int* built_parity_symbols_indices[], int* nb_built_parity_symbols);
   ...
   * @param new_symbol_seqno  (IN)  New symbol's sequence number in {0.. n-1}
   *        range.
  ...
  /**
   * Perform a new decoding step thanks to the newly received symbol.
   * Same as the other DecodingStepWithSymbol method, without the store_symbol
   * argument (prefered solution).
   * @param symbol_canvas (IN-OUT) Global array of received or rebuilt source
   *        symbols (parity symbols need not be stored here).
   *        This is a table of k pointers to buffers.
   *        This array must be cleared (memset(0)) upon
 ...
 protected:
   ...
   */
  void AddToSymbol(void* to, void* from);
...
#define m_nbTotalSymbols (m_nbParitySymbols + m_nbSourceSymbols)
  ...
  // Function called each time a new source symbol is decoded.
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
  void* (*m_allocTmpBuffer_callback)(void* context, int size);
  // Function called each time a temporary buffer is required
  void* (*m_getData_callback)(void* context, void* symbol);
  // Function called each time the data associated to a symbol must be read
  void* (*m_getDataPtrOnly_callback)(void* context, void* symbol);
  // Function called each time we need a ptr to the data buffer associated to a
  // symbol
  ldpc_error_status (*m_storeData_callback)(void* context, void* symbol);
  // Function called each time a symbol's buffer has been updated and must be
  // stored reliably
  ldpc_error_status (*m_freeSymbol_callback)(void* context, void* symbol);
  // Function called each time a symbol (or tmp buffer) can be free'd
#endif  // EXTERNAL_MEMORY_MGMT_SUPPORT
...
inline bool LDPCFecSession::IsInitialized() { return m_initialized; }

inline int LDPCFecSession::GetMaxN() {
...
inline bool LDPCFecSession::IsSourceSymbol(int symbolSeqno) {
  return ((symbolSeqno < m_nbSourceSymbols) ? true : false);
}

inline bool LDPCFecSession::IsParitySymbol(int symbolSeqno) {
  return ((symbolSeqno < m_nbSourceSymbols) ? false : true);
}

inline void* LDPCFecSession::GetBuffer(void* symbol) {
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
...
inline void* LDPCFecSession::GetBufferPtrOnly(void* symbol) {
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
...
inline unsigned int LDPCFecSession::GetNbXor(void) { return m_nbXor; }

inline void LDPCFecSession::ResetNbXor(void) { m_nbXor = 0; }
