...
LDPCFecScheme::LDPCFecScheme() { memset(this, 0, sizeof(*this)); }
...
ldpc_error_status LDPCFecScheme::DetermineSymbolSize(INT64 objectSize,
                                                     int pktSize,
                                                     int* symbolSize,
                                                     int* nbSourceSymbols) {
  ...
  if (nbPkt >= 4000) {
    ...
    if ((pktSize % 2) != 0) {
      fprintf(stderr,
              "LDPCFecScheme::InitScheme: ERROR, pktSize (%d bytes) IS NOT "
              "multiple of 2\n",
              pktSize);
      return LDPC_ERROR;
    }
...
// => See header file for more informations.
ldpc_error_status LDPCFecScheme::InitScheme(int symbolSize, int pktSize) {
  int randInd = 0;
  int backup = 0;

  if (!m_initialized) {
    fprintf(stderr,
            "LDPCFecScheme::InitScheme: ERROR, LDPC Session needs to be "
            "initialized before\n");
    return LDPC_ERROR;
  }

  if (m_SchemeInitialized) {
    fprintf(stderr,
            "LDPCFecScheme::InitScheme: ERROR, Scheme has already been "
            "initialized\n");
    return LDPC_ERROR;
  }
  ...
  if (pktSize < symbolSize) {
    fprintf(stderr,
            "LDPCFecScheme::InitScheme: ERROR, bad packet size %d; must be at "
            "least be equal to the symbol size %d\n",
            pktSize, symbolSize);
    return LDPC_ERROR;
  }
  ...
  m_nbSourcePkts =
      (int)ceil((double)m_nbSourceSymbols / (double)m_nbSymbolsPerPkt);
  m_nbParityPkts =
      (int)ceil((double)m_nbParitySymbols / (double)m_nbSymbolsPerPkt);

  if (m_nbSymbolsPerPkt == 1) {
    if (m_pktSize != m_symbolSize) {
      fprintf(stderr,
              "LDPCFecScheme::InitScheme: ERROR, pktSize %d is not equal to "
              "symbolSize %d\n",
...
// => See header file for more informations.
ldpc_error_status LDPCFecScheme::BuildPkt(int pktIdx, void** pktBuffer,
                                          void* symbol_canvas[],
  ...
  if (m_nbSymbolsPerPkt == 1) {
    ...
    *ESIofFirstSymbol = pktIdx;
    memcpy((*(char**)pktBuffer), symbol_canvas[*ESIofFirstSymbol],
           m_symbolSize);
  } else {
      ...
      *ESIofFirstSymbol = (pktIdx * m_nbSymbolsPerPkt);
      ...
      for (j = 0; j < m_nbSymbolsPerPkt; j++) {
        memcpy((*(char**)pktBuffer) + j * m_symbolSize,
               symbol_canvas[(j + *ESIofFirstSymbol) % m_nbSourceSymbols],
               m_symbolSize);
      }
    } else {
      // generate a parity (repair) packet
      *ESIofFirstSymbol = m_nbSourceSymbols +
  ...
  if (this->m_verbosity >= 1) {
    printf(
        "LDPCFecScheme::BuildPkt: Packet %i build, with ESI %i as first "
        "symbol\n",
...
// => See header file for more informations.
ldpc_error_status LDPCFecScheme::DecomposePkt(void* pktBuffer,
                                              int ESIofFirstSymbol,
    ...
    if (ESIofFirstSymbol < m_nbSourceSymbols) {
      ...
      for (j = 0; j < m_nbSymbolsPerPkt; j++) {
        (*ESIofSymbols)[j] = m_nbSourceSymbols +
            m_txseqToESI[(j +
                          m_ESItoTxseq[ESIofFirstSymbol - m_nbSourceSymbols]) %
                         m_nbParitySymbols];
        (*GeneratedSymbols)[j] = ((char*)pktBuffer) + j * m_symbolSize;
      }
    }
...
// => See header file for more informations.
ldpc_error_status LDPCFecScheme::DecodingStepWithPkt(void* symbol_canvas[],
                                                     void* pktBuffer,
                                                     int ESIofFirstSymbol) {
  ASSERT(pktBuffer);
...
ldpc_error_status LDPCFecScheme::DecodingStepWithPkt(void* symbol_canvas[],
  ...
  err = DecomposePkt(pktBuffer, ESIofFirstSymbol, (void***)&generatedSymbols,
                     &ESIofSymbols);
  if (err == LDPC_ERROR) {
    goto error;
  }
