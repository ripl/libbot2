...
int ldpc_wrapper::init(int objSize_, int pktSize_, double fec_ratio_,
                       int typeFlag_) {
  pktSize = pktSize_;
  objSize = objSize_;
  ...
  if (MyFecScheme->DetermineSymbolSize(objSize, pktSize, &symbolSize,
                                       &nbDATA) == LDPC_ERROR) {
    fprintf(stderr,
            "ERROR: MyFecScheme->DetermineSymbolSize() failed. Check the -ps "
            "value provided.\n");
    exit(1);
  }
  if (fec_ratio < 1.0) {
    fprintf(stderr,
            "ERROR: invalid FEC ratio %.3f (must be >= 1.0).\nMake sure one "
            "has been specified with -fec<n>\n",
            fec_ratio);
    exit(1);
  }
  ...
  if (MyFecScheme->InitSession(nbDATA, nbFEC, symbolSize, typeFlag, 23,
                               TypeTRIANGLE, 3) == LDPC_ERROR) {
...
// initializer for encoder
ldpc_enc_wrapper::ldpc_enc_wrapper(uint8_t* data_to_send, int objSize_,
                                   int packetSize, double fec_rate) {
...
ldpc_dec_wrapper::ldpc_dec_wrapper(int objSize_, int packetSize,
                                   double fec_rate) {
...
int ldpc_dec_wrapper::processPacket(uint8_t* pktBuf, int16_t ESI) {
  ...
  if (packetNum >= nbDATAPkts &&
      MyFecScheme->IsDecodingComplete((void**)data)) {
    return 1;
  }
  if (packetNum >= nbPKT) {
    return -1;
