...
// DecodingStepWithSymbol: Perform a new decoding step with a new (given)
// symbol. This is the legacy front end to the DecodingStepWithSymbol() method.
// The actual work will be done in the other DecodingStepWithSymbol() method.
// => See header file for more informations.
ldpc_error_status LDPCFecSession::DecodingStepWithSymbol(void* symbol_canvas[],
                                                         void* new_symbol,
  ...
  if (store_symbol == false) {
    return (
        DecodingStepWithSymbol(symbol_canvas, new_symbol, new_symbol_seqno));
  }
  ...
  if ((mod2sparse_last_in_col(m_pchkMatrix, GetMatrixCol(new_symbol_seqno))
    ...
    if (this->m_verbosity >= 1) {
      printf(
          "LDPCFecSession::DecodingStepWithSymbol: %s symbol %d already "
          "received or rebuilt, ignored\n",
  ...
  if (IsSourceSymbol(new_symbol_seqno)) {
    ...
    if (this->m_decodedSymbol_callback != NULL) {
      new_symbol_dst = m_decodedSymbol_callback(m_context_4_callback,
                                                m_symbolSize, new_symbol_seqno);
    } else {
    ...
    if (new_symbol_dst == NULL) {
      fprintf(
          stderr,
          "LDPCFecSession::DecodingStepWithSymbol: ERROR, out of memory!\n");
      return LDPC_ERROR;
    }
    // Copy data now
    memcpy(GetBufferPtrOnly(new_symbol_dst), GetBuffer(new_symbol),
           m_symbolSize);
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
    if (m_storeData_callback != NULL) {
      m_storeData_callback(m_context_4_callback, new_symbol_dst);
    }
#endif
  } else {
  ...
  return (
      DecodingStepWithSymbol(symbol_canvas, new_symbol_dst, new_symbol_seqno));
}
...
ldpc_error_status LDPCFecSession::DecodingStepWithSymbol(void* symbol_canvas[],
  ...
  TRACE_LVL(1,
            ("LDPCFecSession::DecodingStepWithSymbol: Processing NEW %s "
             "symbol: seq=%d\n",
             (IsSourceSymbol(new_symbol_seqno)) ? "Source" : "Parity",
             new_symbol_seqno))
  ...
  if (IsSourceSymbol(new_symbol_seqno)) {
    ...
    if (IsDecodingComplete(symbol_canvas)) {
      ...
      TRACE_LVL(
          1,
          ("LDPCFecSession::DecodingStepWithSymbol: decoding is finished!\n"))
      return LDPC_OK;
    }
#ifdef PART_SUM_OPTIMIZATION
    keep_symbol = true;
    ...
    if (m_triangleWithSmallFECRatio) {
      ...
      for (e = mod2sparse_first_in_col(m_pchkMatrix,
        ...
        if (m_checkValues[e->row] == NULL && m_nb_unknown_symbols[e->row] > 2) {
          PS_to_create++;
  ...
  for (e = mod2sparse_first_in_col(m_pchkMatrix,
...
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
      if (m_allocTmpBuffer_callback != NULL) {
        currChk = m_allocTmpBuffer_callback(m_context_4_callback, m_symbolSize);
      } else {
#endif
    ...
    if (currChk != NULL) {
      ...
      if (m_nbSymbols_in_equ[row] > 1) {
        ...
        AddToSymbol(GetBufferPtrOnly(currChk), GetBuffer(new_symbol));
      }
      ...
      if (m_triangleWithSmallFECRatio == false) {
        ...
        for (tmp_e = mod2sparse_first_in_row(m_pchkMatrix, row);
          ...
          if (tmp_symbol != NULL) {
            ...
            AddToSymbol(GetBufferPtrOnly(currChk), GetBuffer(tmp_symbol));
            // delete the entry
            delMe = tmp_e;
            ...
            if (IsParitySymbol(tmp_seqno)) {
              ...
              if (m_nbEqu_for_parity[tmp_seqno - m_nbSourceSymbols] == 0) {
                ...
                if (m_freeSymbol_callback != NULL) {
                  m_freeSymbol_callback(m_context_4_callback, tmp_symbol);
                } else {
#endif
                  free(tmp_symbol);
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
      ...
      // completely updated
      if (m_storeData_callback != NULL) {
        m_storeData_callback(m_context_4_callback, currChk);
      }
#endif
    } else {
    ...
    if (m_nbSymbols_in_equ[row] == 1) {
      ...
      if (CheckOfDeg1 == NULL) {
        ...
        if ((CheckOfDeg1 = (int*)calloc(CheckOfDeg1_listSize, sizeof(int*))) ==
            NULL) {
          goto no_mem;
        }
        ...
        CheckOfDeg1_listSize += 4;
        if ((CheckOfDeg1 = (int*)realloc(
                 CheckOfDeg1, CheckOfDeg1_listSize * sizeof(int*))) == NULL) {
          goto no_mem;
        }
      }
  ...
  for (CheckOfDeg1_nb--; CheckOfDeg1_nb >= 0; CheckOfDeg1_nb--) {
    ...
    if (m_nbSymbols_in_equ[row] == 1) {
      ...
      ASSERT(!mod2sparse_at_end(e) && mod2sparse_at_end(e->right))
      ...
      TRACE_LVL(
          1,
          ("LDPCFecSession::DecodingStepWithSymbol: => REBUILT %s symbol %d\n",
      ...
      if (IsSourceSymbol(decoded_symbol_seqno)) {
...
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
        if (this->m_decodedSymbol_callback != NULL) {
          decoded_symbol_dst = m_decodedSymbol_callback(
              m_context_4_callback, m_symbolSize, decoded_symbol_seqno);
        } else {
#endif
          decoded_symbol_dst = (void*)malloc(m_symbolSize);
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
        }
#endif
        ...
        memcpy(GetBufferPtrOnly(decoded_symbol_dst), GetBuffer(currChk),
               m_symbolSize);
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
        if (m_storeData_callback != NULL) {
          m_storeData_callback(m_context_4_callback, decoded_symbol_dst);
        }
#endif
...
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
        if (m_freeSymbol_callback != NULL) {
          m_freeSymbol_callback(m_context_4_callback, currChk);
        } else {
#endif
          free(currChk);
#ifdef EXTERNAL_MEMORY_MGMT_SUPPORT
        ...
        DecodingStepWithSymbol(symbol_canvas, currChk, decoded_symbol_seqno);
