...
static void usage() {
  printf(
      "usage: bot-lcm-logsplice [OPTIONS] <source_logfile1> <source_logfile2> "
      "[source_logfile3...] <dest_logfile>\n"
      "\n"
      ...
      "  -c CHAN   POSIX regular expression.  Channels matching this "
      "expression\n"
      "            will be copied to the destination logfile.  Defaults to .* "
      "if\n"
      "            left unspecified.\n"
      "  -i        invert the regular expression CHAN, so that only channels "
      "not\n"
...
static void _verbose_entry_summary(gpointer key, gpointer value,
                                   gpointer user_data) {
  printf("%20s: %d\n", (char*)key, *((int*)value));
}
...
int main(int argc, char** argv) {
  ...
  while (1) {
    ...
    if (filterChannels) {
      ...
      copy_to_dest =
          (regmatch == 0 && !invert_regex) || (regmatch != 0 && invert_regex);
    }
    if (copy_to_dest) {
