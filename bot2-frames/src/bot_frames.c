...
static void _dispatch_update_callbacks(BotFrames* bot_frames,
...
static void on_transform_update(const lcm_recv_buf_t* rbuf, const char* channel,
  ...
  frame_handle_t* frame_handle = (frame_handle_t*)g_hash_table_lookup(
      bot_frames->frame_handles_by_channel, channel);
  ...
  _dispatch_update_callbacks(bot_frames, frame_handle->frame_name,
                             frame_handle->relative_to, msg->utime);
}

static void on_pose_update(const lcm_recv_buf_t* rbuf, const char* channel,
...
static void on_frames_update(const lcm_recv_buf_t* rbuf, const char* channel,
                             const bot_frames_update_t* msg, void* user_data) {
  BotFrames* bot_frames = (BotFrames*)user_data;
  g_mutex_lock(bot_frames->mutex);
  BotTrans link_transf;
  bot_trans_set_from_quat_trans(&link_transf, msg->quat, msg->trans);
  ...
  frame_handle_t* frame_handle = (frame_handle_t*)g_hash_table_lookup(
      bot_frames->frame_handles_by_name, msg->frame);
  if (frame_handle == NULL) {
    fprintf(stderr,
            "Received frame update for unknown frame, adding link %s->%s to "
            "BotFrames\n",
            msg->frame, msg->relative_to);
    frame_handle = (frame_handle_t*)calloc(1, sizeof(frame_handle_t));
    frame_handle->ctrans_link = bot_ctrans_link_frames(
        bot_frames->ctrans, msg->frame, msg->relative_to, DEFAULT_HISTORY_LEN);
    bot_ctrans_link_update(frame_handle->ctrans_link, &link_transf, msg->utime);
    frame_handle->was_updated = 1;
    frame_handle->frame_name = strdup(msg->frame);
    frame_handle->relative_to = strdup(msg->relative_to);
    g_hash_table_insert(bot_frames->frame_handles_by_name,
                        (gpointer)frame_handle->frame_name,
                        (gpointer)frame_handle);
    ...
    fprintf(
        ...
        msg->frame, msg->relative_to, frame_handle->relative_to);
  }
  g_mutex_unlock(bot_frames->mutex);
...
BotFrames* bot_frames_new(lcm_t* lcm, BotParam* bot_param) {
  ...
  int num_frames =
      bot_param_get_num_subkeys(self->bot_param, "coordinate_frames");
  if (num_frames <= 0) {
    fprintf(stderr,
            "BotFrames Error: param file does not contain a "
            "'coordinate_frames' block\n");
    bot_frames_destroy(self);
    return NULL;
  }

  int ret = bot_param_get_str(self->bot_param, "coordinate_frames.root_frame",
                              &self->root_name);
  if (ret < 0) {
  ...
  frame_handle_t* root_handle =
      (frame_handle_t*)calloc(1, sizeof(frame_handle_t));
  root_handle->frame_name = strdup(self->root_name);
  root_handle->frame_num = self->num_frames++;
  g_hash_table_insert(self->frame_handles_by_name, (gpointer)self->root_name,
                      (gpointer)root_handle);

  char** frame_names =
      bot_param_get_subkeys(self->bot_param, "coordinate_frames");
  ...
  for (int i = 0; i < num_frames; i++) {
    ...
    snprintf(param_key, sizeof(param_key), "coordinate_frames.%s.relative_to",
             frame_name);
    char* relative_to;
    int ret = bot_param_get_str(self->bot_param, param_key, &relative_to);
    if (ret < 0) {
      fprintf(stderr,
              "BotFrames Error: frame %s does not have a 'relative_to' field "
              "block\n",
              frame_name);
      goto fail;
    }
    ...
    snprintf(param_key, sizeof(param_key), "coordinate_frames.%s.history",
             frame_name);
    int history;
    ...
    snprintf(param_key, sizeof(param_key),
             "coordinate_frames.%s.initial_transform", frame_name);
    if (bot_param_get_num_subkeys(self->bot_param, param_key) != 2) {
      fprintf(stderr,
              "BotFrames Error: frame %s does not have the right number of "
              "fields in the 'initial_transform' block\n",
              frame_name);
      goto fail;
    }
    BotTrans init_trans;
    ...
    if (ret < 0) {
      fprintf(
          stderr,
          "BotFrames Error: could not get 'initial_transform' for frame %s\n",
          frame_name);
      goto fail;
    }

    // create and initialize the link
    BotCTransLink* link = bot_ctrans_link_frames(self->ctrans, frame_name,
                                                 relative_to, history + 1);
    bot_ctrans_link_update(link, &init_trans, 0);

    // add the frame to the hash table
    frame_handle_t* frame_handle = (frame_handle_t*)g_hash_table_lookup(
        self->frame_handles_by_name, frame_name);
    if (frame_handle != NULL) {
    ...
    g_hash_table_insert(self->frame_handles_by_name, (gpointer)frame_name,
                        (gpointer)frame_handle);

    // get the update channel
    char* update_channel = NULL;
    snprintf(param_key, sizeof(param_key),
             "coordinate_frames.%s.update_channel", frame_name);
    ret = bot_param_get_str(self->bot_param, param_key, &update_channel);
    int pose_update_chan = 0;
    if (ret < 0) {
      snprintf(param_key, sizeof(param_key),
               "coordinate_frames.%s.pose_update_channel", frame_name);
      ret = bot_param_get_str(self->bot_param, param_key, &update_channel);
      if (ret == 0) {
        pose_update_chan = 1;
      }
    ...
    if (update_channel != NULL) {
      frame_handle_t* entry = (frame_handle_t*)g_hash_table_lookup(
          self->frame_handles_by_channel, update_channel);
      if (entry != NULL) {
        fprintf(stderr,
                "BotFrames Error: update_channel %s for frame %s already used "
                "for frame %s\n",
      ...
      if (!pose_update_chan) {
        frame_handle->transform_subscription =
            bot_core_rigid_transform_t_subscribe(
                self->lcm, update_channel, on_transform_update, (void*)self);
      } else {
        frame_handle->pose_subscription = bot_core_pose_t_subscribe(
            self->lcm, update_channel, on_pose_update, (void*)self);
      }
      g_hash_table_insert(self->frame_handles_by_channel,
                          (gpointer)update_channel, (gpointer)frame_handle);
    }
  }

  // subscribe to the default update handler
  self->update_subscription = bot_frames_update_t_subscribe(
      self->lcm, BOT_FRAMES_UPDATE_CHANNEL, on_frames_update, (void*)self);
...
void bot_frames_destroy(BotFrames* bot_frames) {
  ...
  if (bot_frames->update_subscription != NULL) {
    bot_frames_update_t_unsubscribe(bot_frames->lcm,
                                    bot_frames->update_subscription);
  }
  ...
  if (bot_frames->update_callbacks != NULL) {
    g_list_foreach(bot_frames->update_callbacks, _update_handler_t_destroy,
                   NULL);
...
void bot_frames_update_frame(BotFrames* bot_frames, const char* frame_name,
  ...
  bot_frames_update_t_publish(bot_frames->lcm, BOT_FRAMES_UPDATE_CHANNEL,
                              &msg);  // lcm object is threadsafe
}
...
void bot_frames_add_update_subscriber(
  ...
  uh->user = user;
  g_mutex_lock(bot_frames->mutex);
  bot_frames->update_callbacks =
      g_list_append(bot_frames->update_callbacks, uh);
  g_mutex_unlock(bot_frames->mutex);
}
...
int bot_frames_get_latest_timestamp(BotFrames* bot_frames,
  ...
  g_mutex_lock(bot_frames->mutex);
  int status = bot_ctrans_get_trans_latest_timestamp(
      bot_frames->ctrans, from_frame, to_frame, timestamp);
  g_mutex_unlock(bot_frames->mutex);
  return status;
}
...
int bot_frames_get_trans_with_utime(BotFrames* bot_frames,
                                    ...
                                    BotTrans* result) {
  g_mutex_lock(bot_frames->mutex);
  int status = bot_ctrans_get_trans(bot_frames->ctrans, from_frame, to_frame,
                                    utime, result);
  g_mutex_unlock(bot_frames->mutex);
  return status;
}
...
int bot_frames_get_trans(BotFrames* bot_frames, const char* from_frame,
  ...
  g_mutex_lock(bot_frames->mutex);
  int status = bot_ctrans_get_trans_latest(bot_frames->ctrans, from_frame,
                                           to_frame, result);
  g_mutex_unlock(bot_frames->mutex);
  return status;
}
...
int bot_frames_get_trans_mat_3x4_with_utime(BotFrames* bot_frames,
                                            ...
                                            double mat[12]) {
  BotTrans bt;
  if (!bot_frames_get_trans_with_utime(bot_frames, from_frame, to_frame, utime,
                                       &bt)) {
    return 0;
  }
  bot_trans_get_mat_3x4(&bt, mat);
...
int bot_frames_get_n_trans(BotFrames* bot_frames, const char* from_frame,
  ...
  g_mutex_lock(bot_frames->mutex);
  BotCTransLink* link =
      bot_ctrans_get_link(bot_frames->ctrans, from_frame, to_frame);
  int n_trans;
  if (!link) {
    n_trans = 0;
  } else {
...
int bot_frames_get_nth_trans(BotFrames* bot_frames, const char* from_frame,
  ...
  if (!link) {
    ...
    status =
        bot_ctrans_link_get_nth_trans(link, nth_from_latest, btrans, timestamp);
    if (status && btrans &&
...
const char* bot_frames_get_relative_to(BotFrames* bot_frames,
  ...
  frame_handle_t* frame_handle = (frame_handle_t*)g_hash_table_lookup(
      bot_frames->frame_handles_by_name, frame_name);
