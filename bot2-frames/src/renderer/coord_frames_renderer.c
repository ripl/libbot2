...
enum { PATH_MODE_NORMAL, PATH_MODE_FLOOR, PATH_MODE_CURTAIN, PATH_MODE_AXES };

// called by bot_ptr_circular when a path sample needs to be freed.
static void free_path_element(void* user, void* p) { free(p); }

static void on_find_button(GtkWidget* button, RendererFrames* self) {
  if (bot_ptr_circular_size(self->path) == 0) {
    return;
  }
  ...
  BotTrans* last = (BotTrans*)bot_ptr_circular_index(self->path, 0);
...
static void destroy_renderer_frames(BotRenderer* super) {
  ...
  bot_ptr_circular_destroy(self->path);
  free(self);
}

static void draw_axis(BotTrans* axis_to_local, float size, float lineThickness,
                      float opacity) {
...
static void draw_path(RendererFrames* self, BotTrans* last_coord) {
  ...
  float* path_color = bot_color_util_jet(
      bot_gtk_param_widget_get_double(self->pw, PARAM_PATH_COLOR));

  switch (bot_gtk_param_widget_get_enum(self->pw, PARAM_PATH_RENDER_MODE)) {
    case PATH_MODE_AXES: {
      draw_axis(last_coord, .5, 2, .4);
      for (unsigned int i = 0;
           i < MIN(bot_ptr_circular_size(self->path), max_draw_poses); i++) {
    ...
    case PATH_MODE_CURTAIN: {
      ...
      int nposes = MIN(bot_ptr_circular_size(self->path), max_draw_poses);
...
static void update_path_hist(RendererFrames* self, BotTrans* frame_to_root) {
  BotTrans t;
  if (bot_ptr_circular_size(self->path) > 0) {
    bot_trans_copy(&t, (BotTrans*)bot_ptr_circular_index(self->path, 0));
  } else {
  ...
  if (dist > bot_gtk_param_widget_get_double(self->pw, PARAM_DECIMATE_PATH) ||
...
static void update_view_follower(RendererFrames* self,
  ...
  double xyzt[4] = {frame_to_root->trans_vec[0], frame_to_root->trans_vec[1],
                    frame_to_root->trans_vec[2], yaw};
...
static void draw(BotViewer* viewer, BotRenderer* super) {
  ...
  int draw_frame_num =
      bot_gtk_param_widget_get_enum(self->pw, PARAM_FRAME_SELECT);
  const char* draw_frame =
      bot_gtk_param_widget_get_enum_str(self->pw, PARAM_FRAME_SELECT);
  if (draw_frame_num == 0) {
    return;
  }

  BotTrans frame_to_root;
  bot_frames_get_trans(self->frames, draw_frame, self->rootFrame,
                       &frame_to_root);

  update_path_hist(self, &frame_to_root);

  draw_path(self, &frame_to_root);

  if (bot_gtk_param_widget_get_bool(self->pw, PARAM_SHOW_FRAME)) {
    draw_axis(&frame_to_root, 1, 4, .7);
  }
...
static void on_param_widget_changed(BotGtkParamWidget* pw, const char* name,
  ...
  if (strcmp(name, PARAM_FRAME_SELECT) == 0) {
...
static void on_load_preferences(BotViewer* viewer, GKeyFile* keyfile,
  ...
  bot_gtk_param_widget_load_from_key_file(self->pw, keyfile,
                                          self->renderer.name);
}
...
void bot_frames_add_named_renderer_to_viewer(BotViewer* viewer,
  ...
  bot_gtk_param_widget_add_enumv(
      self->pw, PARAM_FRAME_SELECT, BOT_GTK_PARAM_WIDGET_DEFAULTS, 0,
      self->numFrames, (const char**)self->frameNames, self->frameNums);
  ...
  bot_gtk_param_widget_add_booleans(self->pw, BOT_GTK_PARAM_WIDGET_CHECKBOX,
                                    PARAM_FOLLOW_POS, 1, PARAM_FOLLOW_YAW, 0,
                                    NULL);

  bot_gtk_param_widget_add_booleans(self->pw, BOT_GTK_PARAM_WIDGET_CHECKBOX,
                                    PARAM_SHOW_FRAME, 1, PARAM_SHOW_SHADOW, 0,
                                    NULL);

  //  self->draw_path_on_floor=0;

  bot_gtk_param_widget_add_enum(
      self->pw, PARAM_PATH_RENDER_MODE, BOT_GTK_PARAM_WIDGET_DEFAULTS, 0,
      "Line", PATH_MODE_NORMAL, "Line On floor", PATH_MODE_FLOOR, "Curtain",
      PATH_MODE_CURTAIN, "Axes", PATH_MODE_AXES, NULL);

  bot_gtk_param_widget_add_double(self->pw, PARAM_PATH_COLOR,
                                  BOT_GTK_PARAM_WIDGET_SLIDER, 0, 1, .01, .3);
  bot_gtk_param_widget_add_double(self->pw, PARAM_DECIMATE_PATH,
                                  BOT_GTK_PARAM_WIDGET_SPINBOX, 0, 100, .01,
                                  .1);

  bot_gtk_param_widget_add_int(self->pw, PARAM_MAXPOSES,
                               BOT_GTK_PARAM_WIDGET_SPINBOX, 0, MAX_HIST, 1,
                               1000);

  GtkWidget* find_button = gtk_button_new_with_label("Find");
  gtk_box_pack_start(GTK_BOX(renderer->widget), find_button, FALSE, FALSE, 0);
  g_signal_connect(G_OBJECT(find_button), "clicked", G_CALLBACK(on_find_button),
                   self);
...
void bot_frames_add_renderer_to_viewer(BotViewer* viewer, int render_priority,
                                       BotFrames* frames) {
  bot_frames_add_named_renderer_to_viewer(viewer, render_priority, frames,
                                          "BOT_FRAMES");
}
