...
public class LCMGL {
    ...
    public synchronized void glBegin(int mode) {
        add1i(LCMGL_BEGIN, mode);
    }
    ...
    public synchronized void glPointSize(float size) {
        add1f(LCMGL_POINTSIZE, size);
    }
    public synchronized void glLineWidth(float size) {
        add1f(LCMGL_LINE_WIDTH, size);
    }

    public synchronized void glColor3f(float r, float g, float b) {
        add3f(LCMGL_COLOR3F, r, g, b);
    }
    public synchronized void glColor4f(float r, float g, float b, float a) {
        add4f(LCMGL_COLOR4F, r, g, b, a);
    }

    public synchronized void glScalef(float x, float y, float z) {
        add3f(LCMGL_SCALEF, x, y, z);
    }
    public synchronized void glTranslated(double x, double y, double z) {
        add3d(LCMGL_TRANSLATED, x, y, z);
    }
    public synchronized void glRotated(double theta, double x, double y, double z) {
        add4d(LCMGL_ROTATED, theta, x, y, z);
    }

    public synchronized void glNormal3f(float x, float y, float z) {
        add3f(LCMGL_NORMAL3F, x, y, z);
    }

    public synchronized void glVertex2d(double x, double y) {
        add2d(LCMGL_VERTEX2D, x, y);
    }
    public synchronized void glVertex2f(float x, float y) {
        add2f(LCMGL_VERTEX2F, x, y);
    }

    public synchronized void glVertex3d(double x, double y, double z) {
        add3d(LCMGL_VERTEX3D, x, y, z);
    }
    public synchronized void glVertex3f(float x, float y, float z) {
        add3f(LCMGL_VERTEX3F, x, y, z);
    }
    ...
    public synchronized void text(double x, double y, double z, String text) {
        this.text(x, y, z, text, 0,
            TEXT_DROP_SHADOW | TEXT_JUSTIFY_CENTER | TEXT_ANCHOR_HCENTER | TEXT_ANCHOR_VCENTER);
    }

    public synchronized void text(double x, double y, double z, String text, int font, int flags) {
        try {
