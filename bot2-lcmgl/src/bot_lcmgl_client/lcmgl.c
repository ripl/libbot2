...
static inline void bot_lcmgl_encode_raw(bot_lcmgl_t* lcmgl, int datalen,
                                        void* data) {
...
void bot_lcmgl_text(bot_lcmgl_t* lcmgl, const double xyz[3], const char* text) {
  bot_lcmgl_text_ex(
      lcmgl, xyz, text, 0,
      BOT_GL_DRAW_TEXT_DROP_SHADOW | BOT_GL_DRAW_TEXT_JUSTIFY_CENTER |
          BOT_GL_DRAW_TEXT_ANCHOR_HCENTER | BOT_GL_DRAW_TEXT_ANCHOR_VCENTER);
}
...
void bot_lcmgl_line(bot_lcmgl_t* lcmgl, double x_start, double y_start,
...
void bot_lcmgl_draw_arrow_3d(bot_lcmgl_t* lcmgl, double length,
  ...
  // apply translations so the drawing is centered at origin along the x axis
  // per bot_gl_draw_arrow_2d
  lcmglPushMatrix();
  lcmglTranslated(-length / 2, 0, 0);
  lcmglRotated(90, 0, 1, 0);

  // draw body
  lcmglCylinder(xyz, body_width, body_width, length - head_length, slices,
                stacks);
...
int bot_lcmgl_texture2d(bot_lcmgl_t* lcmgl, const void* data, int width,
  ...
  switch (compression) {
    ...
    case BOT_LCMGL_COMPRESS_ZLIB: {
      ...
      uLong compressed_buf_size =
          uncompressed_size * 1.01 + 12;  // with extra space for zlib
      Bytef* compressed_buf = (Bytef*)malloc(compressed_buf_size);
      for (int row = 0; row < height; row++) {
        void* row_start = (uint8_t*)data + row * row_stride;
        uLong compressed_size = compressed_buf_size;
        int compress_return =
            compress2((Bytef*)compressed_buf, &compressed_size,
                      (Bytef*)row_start, uncompressed_size, Z_BEST_SPEED);
