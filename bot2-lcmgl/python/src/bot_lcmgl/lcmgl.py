...
#
# bot2-lcmgl is free software: you can redistribute it and/or modify
...
# License along with bot2-lcmgl. If not, see
...
class lcmgl:
    ...
    def text(self, x, y, z, text, flags=0):
        font = 0
        self.data.write(
            struct.pack(">BIIdddI", LCMGL_TEXT_LONG, font, flags, x, y, z,
                        len(text)))
    ...
    def texture2d(self, data, width, height, format, compression):
        ...
        self.data.write(
            struct.pack(">BIIIIII", LCMGL_TEXTURE2D, tex_id, width, height,
                        format, compression, len(data_tosend)))
        self.data.write(data_tosend)

        return tex_id

    def textureDrawQuad(self, tex_id, top_left_xyz, bot_left_xyz,
                        bot_right_xyz, top_right_xyz):

        if tex_id > self.ntextures or tex_id <= 0:
            raise ValueError("Invalid texture ID")
        self.data.write(
            struct.pack(">BIdddddddddddd", LCMGL_TEXTURE_DRAW_QUAD, tex_id,
...
if __name__ == "__main__":
    ...
    img_data = a.tostring()
    print(len(img_data))
    tex_id = g.texture2d(img_data, width, height, LCMGL_LUMINANCE,
                         LCMGL_COMPRESS_NONE)
    g.glColor3f(0, 0, 1)
    g.textureDrawQuad(tex_id, (-10, 10, 0), (-10, -10, 0), (10, -10, 0),
                      (10, 10, 0))
    g.switch_buffer()
