...
static void spherical_distortion_destroy(void* dist) {
  free((SphericalDistortionParams*)dist);
}

static int spherical_undistort_func(const void* data, const double x,
                                    const double y, double ray[3]) {
  ...
  if (denom < CAMERA_EPSILON) {
    return -1;
  }
  double new_r2 = r2 / denom;

  double ratio;
  if (fabs(r2) < CAMERA_EPSILON) {
    ratio = 0;
  } else {
    ratio = new_r2 / r2;
  }
...
static int spherical_distort_func(const void* data, const double ray[3],
                                  double* x, double* y) {
  SphericalDistortionParams* dist = (SphericalDistortionParams*)data;

  if (ray[2] < CAMERA_EPSILON) {
    return -1;
  }
...
BotDistortionObj* bot_spherical_distortion_create(const double a) {
  BotDistortionObj* dist =
      (BotDistortionObj*)calloc(1, sizeof(BotDistortionObj));
  assert(NULL != dist);

  SphericalDistortionParams* params =
      (SphericalDistortionParams*)calloc(1, sizeof(SphericalDistortionParams));
  params->a = a * a * a * a;
  dist->params = (void*)params;

  BotDistortionFuncs* funcs =
      (BotDistortionFuncs*)calloc(1, sizeof(BotDistortionFuncs));
...
static void angular_lookup_distortion_destroy(void* dist) {
...
static int angular_lookup_undistort_func(const void* data, const double x,
                                         const double y, double ray[3]) {
  ...
  if (fabs(theta_in) > dist->dist_max) {
    return -1;
  }
...
BotDistortionObj* bot_angular_lookup_distortion_create(
  ...
  AngularLookupDistortionParams* params =
      (AngularLookupDistortionParams*)calloc(
          1, sizeof(AngularLookupDistortionParams));
...
static void angular_poly_distortion_destroy(void* dist) {
  AngularPolyDistortionParams* params = (AngularPolyDistortionParams*)dist;
  free(params->coeffs);
  free(params);
}

static double angular_poly_evaluate(const AngularPolyDistortionParams* dist,
                                    const double in) {
  double in2 = in * in;
  double out = in;
  double accum = in;
...
static int angular_poly_distort_func(const void* data, const double ray[3],
                                     double* x, double* y) {
  AngularPolyDistortionParams* dist = (AngularPolyDistortionParams*)data;
  double phi_u =
      acos(ray[2] / sqrt(ray[0] * ray[0] + ray[1] * ray[1] + ray[2] * ray[2]));
  double phi_d = phi_u;
...
static int angular_poly_undistort_func(const void* data, const double x,
                                       const double y, double ray[3]) {
  AngularPolyDistortionParams* dist = (AngularPolyDistortionParams*)data;
...
BotDistortionObj* bot_angular_poly_distortion_create(const double* coeffs,
  ...
  AngularPolyDistortionParams* params = (AngularPolyDistortionParams*)calloc(
      1, sizeof(AngularPolyDistortionParams));
...
static int null_distort_func(const void* data, const double ray[3], double* x,
                             double* y) {
  if (ray[2] < CAMERA_EPSILON) {
    return -1;
  }
...
static int null_undistort_func(const void* data, const double x, const double y,
                               double ray[3]) {
  ray[0] = x;
  ray[1] = y;
  ray[2] = 1;
...
static void null_distortion_destroy(void* dist) {}
...
static void plumb_bob_distortion_destroy(void* dist) {
  free((OpenCvDistortionParams*)dist);
}

// Undistort according to plumb bob model
static int plumb_bob_undistort_func(const void* data, const double x,
                                    const double y, double ray[3]) {
...
static int plumb_bob_distort_func(const void* data, const double ray[3],
                                  double* x, double* y) {
  OpenCvDistortionParams* dist = (OpenCvDistortionParams*)data;
...
static void bot_camtrans_compute_matrices(BotCamTrans* self) {
  double pinhole[] = {self->fx, self->skew, self->cx, 0, self->fy,
                      self->cy, 0,          0,        1};
...
void bot_camtrans_destroy(BotCamTrans* self) {
  if (NULL == self) {
    return;
  }
...
const char* bot_camtrans_get_name(const BotCamTrans* self) {
  return self->name;
}

double bot_camtrans_get_skew(const BotCamTrans* self) { return self->skew; }

double bot_camtrans_get_focal_length_x(const BotCamTrans* self) {
  return self->fx;
}
double bot_camtrans_get_focal_length_y(const BotCamTrans* self) {
  return self->fy;
}
double bot_camtrans_get_image_width(const BotCamTrans* self) {
  return self->width;
}
double bot_camtrans_get_image_height(const BotCamTrans* self) {
  return self->height;
}
double bot_camtrans_get_principal_x(const BotCamTrans* self) {
  return self->cx;
}
double bot_camtrans_get_principal_y(const BotCamTrans* self) {
  return self->cy;
}
double bot_camtrans_get_width(const BotCamTrans* self) { return self->width; }
double bot_camtrans_get_height(const BotCamTrans* self) { return self->height; }
...
int bot_camtrans_unproject_pixel(const BotCamTrans* self, double im_x,
  ...
  return self->obj->funcs->undist_func(self->obj->params, temp[0], temp[1],
                                       ray);
}
...
int bot_camtrans_project_point(const BotCamTrans* self, const double p[3],
  ...
  double uxyz[3];
  int rval =
      self->obj->funcs->dist_func(self->obj->params, p, &uxyz[0], &uxyz[1]);
  if (rval == 0) {
    uxyz[2] = 1;
