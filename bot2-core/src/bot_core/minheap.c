...
static inline void _swap_nodes(BotMinheap* mh, int a, int b) {
...
static void fixup(BotMinheap* mh, int parent_ind) {
  if (parent_ind >= mh->nodes->len) {
    return;
  }
...
BotMinheap* bot_minheap_new() { return bot_minheap_sized_new(8); }
...
BotMinheapNode* bot_minheap_add(BotMinheap* mh, void* ptr, double score) {
...
int bot_minheap_size(BotMinheap* mh) { return mh->nodes->len; }
...
gboolean bot_minheap_is_empty(BotMinheap* mh) { return mh->nodes->len == 0; }
