...
int64_t bot_timestamp_seconds(int64_t v) { return v / 1000000; }

int64_t bot_timestamp_useconds(int64_t v) { return v % 1000000; }
...
void bot_timestamp_sync_free(bot_timestamp_sync_state_t* s) { free(s); }
...
int64_t bot_timestamp_sync(bot_timestamp_sync_state_t* s, int64_t dev_ticks,
  ...
  if (time_err > 1000000000LL) {  // 1000 seconds
    fprintf(stderr,
            "Warning: Time sync has drifted by more than 1000 seconds\n");
    s->sync_host_time = host_utime;
    s->dev_ticks_since_sync = 0;
