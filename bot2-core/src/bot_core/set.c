...
BotSet* bot_set_new_full(GHashFunc hash_func, GEqualFunc equal_func,
  ...
  set->hash_table =
      g_hash_table_new_full(hash_func, equal_func, element_destroy_func, NULL);
...
static void _new_union_add(gpointer key, gpointer value, void* user_data) {
...
BotSet* bot_set_new_union(const BotSet* set1, const BotSet* set2) {
  ...
  if (set1->element_destroy_func != set2->element_destroy_func) {
    g_error(
        "Can't union BotSet objects with different element destroy functions");
    return NULL;
  }
...
static void _new_intersect_add(gpointer key, gpointer value, void* user_data) {
  struct _set_intersect_data* d = user_data;
  if (bot_set_contains(d->other_set, key)) {
    bot_set_add(d->new_set, key);
  }
}
...
BotSet* bot_set_new_intersection(const BotSet* set1, const BotSet* set2) {
  ...
  if (set1->element_destroy_func != set2->element_destroy_func) {
    g_error(
        "Can't instersect BotSet objects with different element destroy "
        "functions");
    return NULL;
  }
  BotSet* result = bot_set_new_full(set1->hash_func, set1->equal_func,
...
static void _new_copy_add(gpointer key, gpointer element, gpointer user_data) {
...
static void _maybe_remove_from_set1(gpointer element, gpointer user_data) {
...
static void _foreach_func(gpointer key, gpointer value, gpointer user_data) {
...
static void _get_elements_foreach(gpointer key, gpointer value,
                                  gpointer user_data) {
  g_ptr_array_add((GPtrArray*)user_data, key);
}

GPtrArray* bot_set_get_elements(BotSet* set) {
  GPtrArray* result = g_ptr_array_sized_new(g_hash_table_size(set->hash_table));
