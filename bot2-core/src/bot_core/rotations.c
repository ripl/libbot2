...
static inline int feq(double a, double b) { return fabs(a - b) < 1e-9; }

static inline int qeq(double* q, double a, double b, double c, double d) {
  return feq(q[0], a) && feq(q[1], b) && feq(q[2], c) && feq(q[3], d);
}

static inline int rpyeq(double roll, double pitch, double yaw, double r,
                        double p, double y) {
 ...
 */
inline void bot_quat_mult(double c[4], const double a[4], const double b[4]) {
...
void bot_quat_rotate_to(const double rot[4], const double v[3], double r[3]) {
  double ab = rot[0] * rot[1];
  double ac = rot[0] * rot[2];
  double ad = rot[0] * rot[3];
  double nbb = -rot[1] * rot[1];
  double bc = rot[1] * rot[2];
  double bd = rot[1] * rot[3];
  double ncc = -rot[2] * rot[2];
  double cd = rot[2] * rot[3];
  double ndd = -rot[3] * rot[3];
...
void bot_quat_rotate(const double rot[4], double v[3]) {
  double ab = rot[0] * rot[1];
  double ac = rot[0] * rot[2];
  double ad = rot[0] * rot[3];
  double nbb = -rot[1] * rot[1];
...
void bot_quat_rotate_and_translate(const double rot[4], const double t[3],
                                   const double v[3], double r[3]) {
  double ab = rot[0] * rot[1];
  double ac = rot[0] * rot[2];
  double ad = rot[0] * rot[3];
  double nbb = -rot[1] * rot[1];
 ...
 */
void bot_quat_rotate_rev(const double rot[4], double v[3]) {
  double a[4];
  double b[4];
  double c[4];
...
void bot_roll_pitch_yaw_to_angle_axis(const double rpy[3], double* theta,
                                      double axis[3]) {
  double q[4];
...
void bot_angle_axis_to_roll_pitch_yaw(double theta, const double axis[3],
                                      double rpy[3]) {
  double q[4];
...
int bot_quaternion_test() {
#define FAIL_TEST                                                         \
  {                                                                       \
    fprintf(stderr, "bot_quaternion_test failed at line %d\n", __LINE__); \
    return 0;                                                             \
  }
  ...
  if (!feq(rvec[0], 0) || !feq(rvec[1], 0) || !feq(rvec[2], 1)) {
    FAIL_TEST;
  }

  bot_quat_to_roll_pitch_yaw(q, rpy);

  if (!rpyeq(roll, pitch, yaw, 0, 0, M_PI)) {
    FAIL_TEST;
  }
...
void bot_quat_interpolate(const double q0[4], const double q1[4], double u,
  ...
  if (fabs(sin_omega) < 1e-6) {
    ...
    double s =
        1 / sqrt(SQ(result[0]) + SQ(result[1]) + SQ(result[2]) + SQ(result[3]));
    result[0] *= s;
    result[1] *= s;
    result[2] *= s;
    result[3] *= s;
