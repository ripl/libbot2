...
char* bot_fileutils_get_unique_filename(const char* path, const char* basename,
                                        uint8_t time_prefix,
  ...
  if (time_prefix) {
    ...
    if (path) {
      snprintf(prefix, sizeof(prefix), "%s/%d-%02d-%02d-%s", path,
               ti.tm_year + 1900, ti.tm_mon + 1, ti.tm_mday, basename);
    } else {
      snprintf(prefix, sizeof(prefix), "%d-%02d-%02d-%s", ti.tm_year + 1900,
               ti.tm_mon + 1, ti.tm_mday, basename);
    }
  } else {
    if (path) {
  ...
  do {
    struct stat statbuf;
    if (extension) {
      snprintf(filename, maxlen, "%s.%02d.%s", prefix, filenum, extension);
    } else {
...
int bot_fileutils_read_fully_timeout(int fd, void* bufin, int len,
                                     int msTimeout) {
  char* buf = (char*)bufin;
  int readsofar = 0;
  int thisread = 0;
  ...
  while (readsofar < len) {
    thisread = bot_fileutils_read_timeout(fd, &buf[readsofar], len - readsofar,
                                          msTimeout);
    if (thisread == 0) {
      return readsofar;
    }
...
int bot_fileutils_read_line_timeout(int fd, void* buf_in, int maxlen,
                                    int msTimeout) {
  int timed_out;
  return bot_fileutils_read_line_timeout_ex(fd, buf_in, maxlen, msTimeout,
                                            &timed_out);
}

int bot_fileutils_read_line_timeout_ex(int fd, void* buf_in, int maxlen,
                                       int msTimeout, int* timed_out) {
  int len = 0;
  int thislen;
...
int bot_fileutils_fwrite64(FILE* f, int64_t v64) {
  int32_t v = v64 >> 32;
  if (0 != bot_fileutils_fwrite32(f, v)) {
    return -1;
  }
  v = v64 & 0xffffffff;
