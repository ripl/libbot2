...
int bot_ringbuf_peek(BotRingBuf* cbuf, int numBytes,
                     uint8_t* buf);  // read numBytes from start of buffer, but
                                     // don't move readPtr

// Return a pointer to a contiguous buffer with the next numBytes of data to be
// read
...
// Get the amount of data currently stored in the buffer (not the allocated
// size)
