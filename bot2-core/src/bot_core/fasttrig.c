...
static inline void bot_fasttrig_sincos_coarse(double theta, double* s,
                                              double* c) {
...
static inline double bot_fasttrig_atan2_coarse(double y, double x) {
  double abs_y = fabs(y);
  double angle;
...
static inline void bot_fasttrig_sincos_test() {
  bot_fasttrig_init();
  ...
  for (int iters = 0; iters < limit; iters++) {
    ...
    double s_err = fabs(s - s2);
    double c_err = fabs(c - c2);
    if (s_err > eps) {
...
static inline float randf() { return ((float)rand()) / (RAND_MAX + 1.0); }
static inline float signed_randf() { return randf() * 2 - 1; }
#define to_degrees(x) ((x) * (180.0 / M_PI))

static inline void bot_fasttrig_atan2_test() {
  double max_err = 0;
