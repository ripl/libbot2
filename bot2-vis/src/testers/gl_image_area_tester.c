...
static gboolean on_gl_area_expose(GtkWidget* widget, GdkEventExpose* event,
                                  void* user_data) {
...
static app_t* app_create(const app_params_t* params) {
  app_t* self = (app_t*)calloc(1, sizeof(app_t));

  self->foo = params->foo;

  if (0 != load_ppm(self, params->filename)) {
    goto fail;
  }

  setup_gui(self);

  bot_gtk_gl_image_area_set_image_format(self->gl_area, self->img_width,
                                         self->img_height, GL_RGB);
...
static void setup_gui(app_t* self) {
  ...
  g_signal_connect(G_OBJECT(main_window), "delete_event", gtk_main_quit, NULL);
  ...
  gtk_widget_set_size_request(GTK_WIDGET(self->gl_area), self->img_width,
                              self->img_height);

  gtk_box_pack_start(GTK_BOX(hbox), GTK_WIDGET(self->gl_area), TRUE, TRUE, 0);

  self->param_widget = BOT_GTK_PARAM_WIDGET(bot_gtk_param_widget_new());
  gtk_box_pack_start(GTK_BOX(hbox), GTK_WIDGET(self->param_widget), FALSE, TRUE,
                     0);
  bot_gtk_param_widget_add_booleans(self->param_widget, 0, "Draw X", 1, NULL);
...
static int ppm_read(FILE* fp, uint8_t** pixels, int* width, int* height,
                    int* rowstride) {
  int maxval;
  int i;
  int nread;
...
static void app_run(app_t* self) { gtk_main(); }

int main(int argc, char** argv) {
  gtk_init(&argc, &argv);

  app_params_t params = {.foo = 1, .bar = NULL, .filename = NULL};

  GOptionEntry options[] = {
      {"foo", 'f', 0, G_OPTION_ARG_INT, &params.foo, "Example int parameter",
       "F"},
