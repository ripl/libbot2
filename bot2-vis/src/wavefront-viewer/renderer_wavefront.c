...
static void _renderer_free(BotRenderer* super) {
...
static void draw_wavefront_model(RendererWavefront* self) {
  glPushMatrix();
  glEnable(GL_BLEND);
...
static GLuint compile_display_list(RendererWavefront* self, char* prefix,
                                   BotWavefrontModel* model) {
...
static void _renderer_draw(BotViewer* viewer, BotRenderer* super) {
  RendererWavefront* self = (RendererWavefront*)super->user;
  ...
  if (!self->display_lists_ready) {
    if (self->wavefront_model)
      self->wavefront_dl =
          compile_display_list(self, "wavefront", self->wavefront_model);
...
void setup_renderer_wavefront(BotViewer* viewer, int render_priority,
                              const char* wavefront_fname) {
  RendererWavefront* self =
      (RendererWavefront*)calloc(1, sizeof(RendererWavefront));
  ...
  bot_gtk_param_widget_add_double(self->pw, PARAM_SCALE,
                                  BOT_GTK_PARAM_WIDGET_SPINBOX, 0, 99999999999,
                                  0.00000001, default_scale);

  bot_gtk_param_widget_add_double(self->pw, PARAM_DX,
                                  BOT_GTK_PARAM_WIDGET_SPINBOX, -9999999999,
                                  99999999999, 0.01, default_dx);
  ...
  bot_gtk_param_widget_add_double(self->pw, PARAM_DZ,
                                  BOT_GTK_PARAM_WIDGET_SPINBOX, -9999999999,
                                  99999999999, 0.01, default_dz);

  bot_gtk_param_widget_add_double(self->pw, PARAM_ROT_X,
                                  BOT_GTK_PARAM_WIDGET_SPINBOX, -180, 180, 0.1,
                                  default_rot_x);
