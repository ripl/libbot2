...
static param_data_t* param_data_new(const char* name, GtkWidget* widget,
                                    GType data_type) {
...
static void bot_gtk_param_widget_class_init(BotGtkParamWidgetClass* klass) {
  ...
  bot_gtk_param_widget_signals[CHANGED_SIGNAL] = g_signal_new(
      "changed", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_FIRST | G_SIGNAL_ACTION,
...
static void bot_gtk_param_widget_init(BotGtkParamWidget* pw) {
  ...
  pw->widget_to_param = g_hash_table_new_full(
      g_direct_hash, g_direct_equal, NULL, (GDestroyNotify)param_data_free);
  pw->estr = NULL;
}
...
static inline param_data_t* get_param_data(BotGtkParamWidget* pw,
                                           const char* name) {
  GtkWidget* w = g_hash_table_lookup(pw->params, name);
  if (!w) {
    return NULL;
  }
  param_data_t* pd = (param_data_t*)g_hash_table_lookup(pw->widget_to_param, w);
  return pd;
}

static inline param_data_t* get_param_data_check_type(BotGtkParamWidget* pw,
                                                      const char* name,
                                                      GType data_type) {
  GtkWidget* w = g_hash_table_lookup(pw->params, name);
  if (!w) {
...
static int have_parameter_key(BotGtkParamWidget* pw, const char* name) {
...
static void generic_widget_changed(GtkWidget* w, gpointer user_data) {
  ...
  param_data_t* pd = (param_data_t*)g_hash_table_lookup(pw->widget_to_param, w);
  g_signal_emit(G_OBJECT(pw), bot_gtk_param_widget_signals[CHANGED_SIGNAL], 0,
                pd->name);
}

static param_data_t* add_row(BotGtkParamWidget* pw, const char* name,
...
int bot_gtk_param_widget_add_int(BotGtkParamWidget* pw, const char* name,
  ...
  switch (ui_hints) {
    ...
    case BOT_GTK_PARAM_WIDGET_DEFAULTS:
      w = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, min, max,
                                   increment);
...
static int add_checkboxes_helper(BotGtkParamWidget* pw, GtkBox* box,
  ...
  g_signal_connect(G_OBJECT(cb), "toggled", G_CALLBACK(generic_widget_changed),
                   pw);
  return 0;
}

static int add_checkboxes(BotGtkParamWidget* pw, const char* name1,
                          int initally_checked1, va_list ap) {
...
static int add_togglebuttons_helper(BotGtkParamWidget* pw, GtkBox* box,
                                    const char* name, int checked) {
  if (have_parameter_key(pw, name)) {
    return -1;
  }
  ...
  g_signal_connect(G_OBJECT(tb), "toggled", G_CALLBACK(generic_widget_changed),
                   pw);
  return 0;
}

static int add_togglebuttons(BotGtkParamWidget* pw, const char* name1,
                             int initally_checked1, va_list ap) {
  GtkBox* hb = GTK_BOX(gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0));
...
static int add_buttons_helper(BotGtkParamWidget* pw, GtkBox* box,
                              const char* name) {
  if (have_parameter_key(pw, name)) {
    return -1;
  }
  ...
  g_signal_connect(G_OBJECT(b), "clicked", G_CALLBACK(generic_widget_changed),
                   pw);
  return 0;
}

static int add_buttons(BotGtkParamWidget* pw, const char* name1, va_list ap) {
  GtkBox* hb = GTK_BOX(gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0));
...
static int _add_menu(BotGtkParamWidget* pw, const char* name, int initial_value,
...
int bot_gtk_param_widget_add_enumv(BotGtkParamWidget* pw, const char* name,
  ...
  switch (ui_hints) {
    ...
    case BOT_GTK_PARAM_WIDGET_DEFAULTS:
      return _add_menu(pw, name, initial_value, noptions, names, values);
      break;
    default:
...
int bot_gtk_param_widget_add_enum(BotGtkParamWidget* pw, const char* name,
  ...
  int status = bot_gtk_param_widget_add_enumv(
      pw, name, ui_hints, initial_value, names->len, (const char**)names->pdata,
      (int*)values->data);
...
int bot_gtk_param_widget_get_int(BotGtkParamWidget* pw, const char* name) {
  ...
  if (GTK_TYPE_SCALE == type) {
    ...
    fprintf(stderr,
            "param_widget:  can't retrieve parameter [%s] "
            "as integer.\n",
            name);
    return 0;
  }
...
double bot_gtk_param_widget_get_double(BotGtkParamWidget* pw,
                                       const char* name) {
  if (!have_parameter_key(pw, name)) {
    fprintf(stderr, "param_widget: invalid parameter [%s]\n", name);
    return 0;
...
static gboolean activate_if_matched(GtkTreeModel* model, GtkTreePath* path,
...
void bot_gtk_param_widget_set_enum(BotGtkParamWidget* pw, const char* name,
  ...
  if (GTK_TYPE_COMBO_BOX == type) {
    GtkTreeModel* model = gtk_combo_box_get_model(GTK_COMBO_BOX(w));
    tree_model_match_data_t matchdata = {val, GTK_COMBO_BOX(w)};
...
static void _param_load_from_keyfile(void* key, void* value, void* user_data) {
...
void bot_gtk_param_widget_load_from_key_file(BotGtkParamWidget* pw,
  ...
  _keyfile_data_t data = {
      .keyfile = keyfile, .group_name = group_name, .pw = pw};
  g_hash_table_foreach(pw->params, _param_load_from_keyfile, &data);
}

static void _param_save_to_keyfile(void* key, void* value, void* user_data) {
  GtkWidget* w = GTK_WIDGET(value);
  _keyfile_data_t* data = user_data;
  const char* param_name = key;
  const char* type = g_object_get_data(G_OBJECT(w), "data-type");
  if (!strcmp(type, "boolean")) {
    ...
    g_key_file_set_boolean(data->keyfile, data->group_name, param_name, val);
  } else if (!strcmp(type, "double")) {
    double val = bot_gtk_param_widget_get_double(data->pw, param_name);
    g_key_file_set_double(data->keyfile, data->group_name, param_name, val);
  } else if (!strcmp(type, "int")) {
...
int bot_gtk_param_widget_modify_int(BotGtkParamWidget* pw, const char* name,
  ...
  if (GTK_TYPE_SCALE == type) {
    ...
    fprintf(stderr,
            "param_widget:  can't modify parameter [%s] "
            "as integer.\n",
            name);
    return -1;
  }
