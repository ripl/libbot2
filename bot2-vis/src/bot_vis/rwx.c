...
static int parse_int(tokenize_t* t, int* d, int radix) {
  char* endptr = NULL;
...
static int parse_double(tokenize_t* t, double* f) {
  char* endptr = NULL;
...
static void parse_require(tokenize_t* t, char* tok) {
...
static int parse_vertex(tokenize_t* tok, BotRwxVertex* v) {
  tokenize_next(tok);
...
static int parse_triangle(tokenize_t* tok, BotRwxTriangle* t) {
  tokenize_next(tok);
...
static int parse_clump(BotRwxClump* clump, tokenize_t* tok) {
  // get clump name
  ...
  while (1) {
    if (!strcmp(tok->token, "Triangle")) {
      ...
      if (clump->ntriangles >= ntbuf) {
        ntbuf += 10000;
        tbuf = (BotRwxTriangle*)realloc(tbuf, ntbuf * sizeof(BotRwxTriangle));
      }
...
static inline void _matrix_vector_multiply_4x4_3d2(const double m[16],
                                                   const double v[3],
                                                   double result[3]) {
...
void bot_rwx_model_gl_draw(BotRwxModel* model) {
  ...
  for (citer = model->clumps; citer != NULL; citer = citer->next) {
    ...
    for (i = 0; i < clump->ntriangles; i++) {
      ...
      // render the triangle
      glNormal3d(normals[vid1 * 3 + 0], normals[vid1 * 3 + 1],
                 normals[vid1 * 3 + 2]);
      glVertex3f(v1->pos[0], v1->pos[1], v1->pos[2]);
      glNormal3d(normals[vid2 * 3 + 0], normals[vid2 * 3 + 1],
                 normals[vid2 * 3 + 2]);
      glVertex3f(v2->pos[0], v2->pos[1], v2->pos[2]);
      glNormal3d(normals[vid3 * 3 + 0], normals[vid3 * 3 + 1],
                 normals[vid3 * 3 + 2]);
