...
void bot_gl_texture_draw_coords(BotGlTexture* t, double x0, double y0,
...
int bot_gl_texture_upload(BotGlTexture* t, GLenum format, GLenum type,
  ...
  if (t->use_pbo) {
    ...
    glTexImage2D(t->target, 0, t->int_format, t->width, t->height, 0, format,
                 type, 0);
...
GLuint bot_gl_texture_get_texname(BotGlTexture* t) { return t->texname; }

GLenum bot_gl_texture_get_target(BotGlTexture* t) { return t->target; }
