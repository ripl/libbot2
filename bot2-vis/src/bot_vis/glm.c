...
static GLfloat* glmWeldVectors(GLfloat* vectors, GLuint* numvectors,
                               GLfloat epsilon) {
  GLfloat* copies;
  GLuint copied;
  GLuint i, j;
...
static GLuint glmFindOrAddTexture(GLMmodel* model, const char* name) {
  GLuint i;
  char *dir, *filename;
  float width, height;
  ...
  // texture (0).
  model->numtextures++;
  model->textures = (GLMtexture*)realloc(
      model->textures, sizeof(GLMtexture) * model->numtextures);
  model->textures[model->numtextures - 1].name = strdup(name);
  model->textures[model->numtextures - 1].id = glmLoadTexture(
      filename, GL_TRUE, GL_TRUE, GL_TRUE, GL_TRUE, &width, &height);
  model->textures[model->numtextures - 1].width = width;
  model->textures[model->numtextures - 1].height = height;
  DBG_(__glmWarning("allocated texture %d (id=%d,width=%g,height=%g)",
                    model->numtextures - 1,
                    model->textures[model->numtextures - 1].id, width, height));
    ...
    switch (buf[0]) {
      ...
      case 'n':  // newmtl
        if (strncmp(buf, "newmtl", 6) != 0)
          __glmFatalError(
              "glmReadMTL: Got \"%s\" instead of \"newmtl\" in file \"%s\"",
              buf, filename);
        rem = fgets(buf, sizeof(buf), file);
        nummaterials++;
    ...
    switch (buf[0]) {
      ...
      case 'i':
        if (strncmp(buf, "illum", 5) != 0) {
          __glmFatalError(
              "glmReadMTL: Got \"%s\" instead of \"illum\" in file \"%s\"", buf,
              filename);
        }
      ...
      case 'm':
        ...
        if (strncmp(buf, "map_Kd", 6) == 0) {
          model->materials[nummaterials].map_diffuse =
              glmFindOrAddTexture(model, t_filename);
          free(t_filename);
        } else {
...
static GLvoid glmWriteMTL(GLMmodel* model, char* modelpath, char* mtllibname) {
  ...
  if (!file) {
    __glmFatalError("glmWriteMTL() failed: can't open file \"%s\".", filename);
  }
  free(filename);
  ...
  while (fscanf(file, "%s", buf) != EOF) {
    switch (buf[0]) {
      ...
      case 'v':  // v, vn, vt
        switch (buf[1]) {
          case '\0':  // vertex
            remi = fscanf(file, "%f %f %f", &vertices[3 * numvertices + 0],
                          &vertices[3 * numvertices + 1],
                          &vertices[3 * numvertices + 2]);
            numvertices++;
            break;
          case 'n':  // normal
            remi = fscanf(file, "%f %f %f", &normals[3 * numnormals + 0],
                          &normals[3 * numnormals + 1],
                          &normals[3 * numnormals + 2]);
            numnormals++;
            break;
          case 't':  // texcoord
            remi = fscanf(file, "%f %f", &texcoords[2 * numtexcoords + 0],
...
GLvoid glmFacetNormals(GLMmodel* model) {
  ...
  model->facetnorms =
      (GLfloat*)malloc(sizeof(GLfloat) * 3 * (model->numfacetnorms + 1));
...
GLvoid glmVertexNormals(GLMmodel* model, GLfloat angle,
  ...
  for (i = 1; i <= model->numvertices; i++) {
    ...
    while (node) {
      ...
      if ((T(node->index).findex != -1) &&
          (T(members[i]->index).findex != -1)) {
      ...
      if (node->averaged) {
        ...
        for (j = 0; j < 3; j++) {
          ...
          if (T(node->index).vindices[j] == i) {
            assert(T(node->index).nindices[j] == -1 ||
                   T(node->index).nindices[j] <= model->numnormals);
        ...
        while (model->numnormals < numnormals) {
          ...
          model->numnormals += 100;
          model->normals = (GLfloat*)realloc(
              model->normals, sizeof(GLfloat) * 3 * (model->numnormals + 1));
        }
        assert(T(node->index).findex == -1 ||
               T(node->index).findex <= model->numfacetnorms);
        assert(model->numnormals >= numnormals);
...
GLvoid glmLinearTexture(GLMmodel* model) {
  ...
  scalefactor =
      2.0 / glmAbs(glmMax(glmMax(dimensions[0], dimensions[1]), dimensions[2]));
...
GLMmodel* glmReadOBJ(const char* filename) {
  ...
  // allocate memory
  model->vertices =
      (GLfloat*)malloc(sizeof(GLfloat) * 3 * (model->numvertices + 1));
  model->triangles =
      (GLMtriangle*)malloc(sizeof(GLMtriangle) * model->numtriangles);
  if (model->numnormals) {
    model->normals =
        (GLfloat*)malloc(sizeof(GLfloat) * 3 * (model->numnormals + 1));
  }
    ...
    for (j = 0; j < 3; j++) {
      if (T(i).nindices[j] != -1)
        if (T(i).nindices[j] <= 0 || T(i).nindices[j] > model->numnormals)
          __glmFatalError(
              "normal index for triangle %d out of bounds (%d > %d)\n", i,
              T(i).nindices[j], model->numnormals);
      if (T(i).vindices[j] != -1)
        if (T(i).vindices[j] <= 0 || T(i).vindices[j] > model->numvertices)
          __glmFatalError(
              "vertex index for triangle %d out of bounds (%d > %d)\n", i,
              T(i).vindices[j], model->numvertices);
    }
  }
...
GLvoid glmWriteOBJ(GLMmodel* model, char* filename, GLuint mode) {
  ...
  while (group) {
    ...
    for (i = 0; i < group->numtriangles; i++) {
#ifdef MATERIAL_BY_FACE
      if (T(group->triangles[i]).material &&
          T(group->triangles[i]).material != material) {
      ...
      if (mode & GLM_SMOOTH && mode & GLM_TEXTURE) {
        ...
        fprintf(
            file, "f %d/%d %d/%d %d/%d\n", T(group->triangles[i]).vindices[0],
            T(group->triangles[i]).findex, T(group->triangles[i]).vindices[1],
            T(group->triangles[i]).findex, T(group->triangles[i]).vindices[2],
        ...
        fprintf(
            ...
            T(group->triangles[i]).vindices[1], T(group->triangles[i]).findex,
            T(group->triangles[i]).vindices[2], T(group->triangles[i]).findex);
      } else {
...
#ifdef MATERIAL_BY_FACE
        if (mode & (GLM_MATERIAL | GLM_COLOR | GLM_TEXTURE)) {
          // if the triangle has a different material than the last drawn
          // triangle
        ...
        if (blending == blenditer) {
            ...
            if (mode & GLM_TEXTURE) {
              if (newtexture) {
                ...
                if (map_diffuse == -1) {
                  ...
                  glBindTexture(_glmTextureTarget,
                                model->textures[map_diffuse].id);
                }
                glBegin(GL_TRIANGLES);
              }
            }
          ...
          for (j = 0; j < 3; j++) {
            if (mode & GLM_SMOOTH && (triangle->nindices[j] != -1)) {
              assert(triangle->nindices[j] >= 1 &&
                     triangle->nindices[j] <= model->numnormals);
            ...
            if (mode & GLM_TEXTURE && (triangle->tindices[j] != -1) &&
              ...
              assert(triangle->tindices[j] >= 1 &&
                     triangle->tindices[j] <= model->numtexcoords);
              glTexCoord2f(model->texcoords[2 * triangle->tindices[j]] *
                               model->textures[map_diffuse].width,
                           model->texcoords[2 * triangle->tindices[j] + 1] *
                               model->textures[map_diffuse].height);
            }
            assert(triangle->vindices[j] >= 1 &&
                   triangle->vindices[j] <= model->numvertices);
...
GLvoid glmFlipTexture(unsigned char* texture, int width, int height) {
  ...
  for (int i = 0; i < height / 2; i++) {
    memcpy(buf, (&texture[3 * width * (height - 1 - i)]),
           3 * width * sizeof(unsigned char));
    memcpy((&texture[3 * width * (height - 1 - i)]), (&texture[3 * width * i]),
           3 * width * sizeof(unsigned char));
...
GLvoid glmFlipModelTextures(GLMmodel* model) {
  ...
  while (group) {
    ...
    if (material->image) {
      ...
      glBindTexture(_glmTextureTarget,
                    model->materials[group->material].t_id[0]);

      gluBuild2DMipmaps(
          _glmTextureTarget, 3, model->materials[group->material].width,
          model->materials[group->material].height, GL_RGB, GL_UNSIGNED_BYTE,
          model->materials[group->material].image);
    }
