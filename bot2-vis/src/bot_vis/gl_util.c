...
void bot_gl_draw_circle(double r) {
  ...
  for (int i = 0; i < sizeof(_circle_points) / (2 * sizeof(float));
       i += decimate) {
...
void bot_gl_draw_ellipse(double a, double b, double angle, int steps) {
...
void bot_gl_draw_ortho_circles_3d() {
...
void bot_gl_draw_arrow_3d(double length, double head_width, double head_length,
                          double body_width) {
  int slices = 20;
  int stacks = 20;
...
void bot_gl_draw_text(const double xyz[3], void* font, const char* text,
  ...
  if (flags & BOT_GL_DRAW_TEXT_NORMALIZED_SCREEN_COORDINATES) {
    ...
    double bogus;
    if (!gluProject(xyz[0], xyz[1], xyz[2], model_matrix, proj_matrix, viewport,
  ...
  if (flags & BOT_GL_DRAW_TEXT_DROP_SHADOW) {
    ...
    double x0 = winxy[0] - max_width / 2 - hmargin,
           x1 = winxy[0] + max_width / 2 + hmargin;
    double y0 = winxy[1] - height / 2.0 - vmargin_bottom,
           y1 = winxy[1] + height / 2.0 + vmargin_top;
    glVertex2f(x0, y0);
    glVertex2f(x0, y1);
    glVertex2f(x1, y1);
    glVertex2f(x1, y0);
...
void bot_gl_print_current_matrix(void) {
  ...
  printf("  %8.3f %8.3f %8.3f %8.3f\n", mtx_val[0], mtx_val[4], mtx_val[8],
         mtx_val[12]);
  printf("  %8.3f %8.3f %8.3f %8.3f\n", mtx_val[1], mtx_val[5], mtx_val[9],
         mtx_val[13]);
  printf("  %8.3f %8.3f %8.3f %8.3f\n", mtx_val[2], mtx_val[6], mtx_val[10],
         mtx_val[14]);
  printf("  %8.3f %8.3f %8.3f %8.3f\n", mtx_val[3], mtx_val[7], mtx_val[11],
         mtx_val[15]);
}
