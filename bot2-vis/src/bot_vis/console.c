...
static _line_t* _line_new(const char* text, const double rgba[4],
                          int64_t utime) {
...
static void _line_free(_line_t* pr) {
  free(pr->text);
...
BotGlConsole* bot_gl_console_new() {
...
void bot_gl_console_printf(BotGlConsole* console, const char* format, ...) {
  ...
  for (int i = 0; i < text_len; i++) {
    ...
    if (!last_line || last_line->terminated) {
      ...
      char* combined =
          g_strdup_printf("%s%s", last_line->text, text + line_start);
      free(last_line->text);
  ...
  if (line_start < text_len - 1) {
    _line_t* last_line = _line_new(text + line_start, console->cur_rgba, now);
    g_queue_push_head(console->lines, last_line);
  }
  free(text);
}

void bot_gl_console_render(BotGlConsole* console, double elapsed) {
