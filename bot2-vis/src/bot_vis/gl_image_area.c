...
G_DEFINE_TYPE(BotGtkGlImageArea, bot_gtk_gl_image_area,
              BOT_GTK_TYPE_GL_DRAWING_AREA);
...
static void bot_gtk_gl_image_area_init(BotGtkGlImageArea* self) {
  self->target = 0;
  self->int_format = 0;
  self->format = 0;
  self->texname = 0;
  ...
  g_signal_connect(G_OBJECT(self), "expose-event", G_CALLBACK(on_gl_expose),
                   self);
  g_signal_connect_after(G_OBJECT(self), "expose-event",
                         G_CALLBACK(on_gl_expose_after), self);
}

static void bot_gtk_gl_image_area_class_init(BotGtkGlImageAreaClass* klass) {
...
GtkWidget* bot_gtk_gl_image_area_new() {
...
static gboolean on_gl_expose(GtkWidget* widget, GdkEventExpose* event,
  ...
  if (self->width <= 0 || self->height <= 0 || !self->texname) {
    return FALSE;
  }
...
int bot_gtk_gl_image_area_set_image_format(BotGtkGlImageArea* self, int width,
                                           int height, GLenum format) {
  if (format != GL_LUMINANCE && format != GL_RGB && format != GL_BGR &&
  ...
  if (has_non_power_of_two) {
    ...
    fprintf(stderr,
            "Error: GL supports neither non-power-of-two nor "
            "texture-rectangle\n");
    return -1;
  }
...
int bot_gtk_gl_image_area_upload_image(BotGtkGlImageArea* self,
  ...
  if (self->use_pbo && (row_stride * self->height) > self->max_data_size) {
    fprintf(stderr,
            "Error: gl_texture buffer (%d bytes) too small for "
            "texture (%d bytes)\n",
            self->max_data_size, row_stride * self->height);
    return -1;
  }
  ...
  if (self->use_pbo) {
    ...
    if (data) {
      uint8_t* buffer_data =
          (uint8_t*)glMapBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, GL_WRITE_ONLY);
      memcpy(buffer_data, data, row_stride * self->height);
      glUnmapBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB);
    }

    glTexImage2D(self->target, 0, self->int_format, self->width, self->height,
                 0, self->format, type, 0);

    glBindBufferARB(GL_PIXEL_UNPACK_BUFFER_ARB, 0);
  } else {
    glTexImage2D(self->target, 0, self->int_format, self->width, self->height,
                 0, self->format, type, data);
  }
