...
static GLubyte* glmReadPPM(const char* filename, GLboolean alpha, int* width,
                           int* height, int* type) {
  FILE* fp;
  int i, w, h, d;
  unsigned char* image;
...
GLuint glmLoadTexture(const char* filename, GLboolean alpha, GLboolean repeat,
                      ...
                      GLfloat* texcoordwidth, GLfloat* texcoordheight) {
  GLuint tex;
...
DONE:
  ...
  if (gl_max_texture_size == 0) {
    DBG_(__glmWarning(
        "glmLoadTexture(): Maximum texture size is %d. Skipping texture!",
        gl_max_texture_size));
  ...
  if ((width != xSize2) || (height != ySize2)) {
    ...
    retval = gluScaleImage(type, width, height, GL_UNSIGNED_BYTE, data, xSize2,
                           ySize2, GL_UNSIGNED_BYTE, rdata);
  ...
  glTexParameteri(_glmTextureTarget, GL_TEXTURE_WRAP_S,
                  (repeat) ? GL_REPEAT : GL_CLAMP);
  glTexParameteri(_glmTextureTarget, GL_TEXTURE_WRAP_T,
                  (repeat) ? GL_REPEAT : GL_CLAMP);
  if (mipmaps && _glmTextureTarget == GL_TEXTURE_2D) {
