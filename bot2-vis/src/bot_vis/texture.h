...
#ifndef BOT2_VIS_BOT_VIS_TEXTURE_H_
#define BOT2_VIS_BOT_VIS_TEXTURE_H_
 ...
 * @param max_data_size maximum number of bytes required to store the texture
 * data in memory.
 *
...
/**
 * Destructor.  The same OpenGL context used to allocated the texture must be
 * active.
 */
 ...
 *    GL_BGRA.
 * @param type OpenGL data type used to represent the texture pixels.  Supported
 * types are: GL_UNSIGNED_BYTE, GL_BYTE, GL_UNSIGNED_SHORT, GL_SHORT,
 * GL_UNSIGNED_INT, GL_INT, GL_FLOAT.
 * @param stride number of bytes separating the start of each row in the texture
 * data.
 * @param data the texture data.
 *
 * Uploads texture data to OpenGL-managed memory.  The original data is not
 * needed for rendering after an upload.
 */
int bot_gl_texture_upload(BotGlTexture* t, GLenum format, GLenum type,
 ...
 */
void bot_gl_texture_draw(BotGlTexture* t);

/**
 * Renders the texture in a quadrilateral using the specified coordinates
 */
void bot_gl_texture_draw_coords(BotGlTexture* t, double x_top_left,
                                double y_top_left, double z_top_left,
                                double x_bot_left, double y_bot_left,
                                double z_bot_left, double x_bot_right,
                                double y_bot_right, double z_bot_right,
                                double x_top_right, double y_top_right,
                                double z_top_right);

/**
 * Sets the rendering interpolation mode.
 * @param nearest_or_linear typically GL_LINEAR or GL_NEAREST.  default is
 * GL_LINEAR
 *
...
#endif  // BOT2_VIS_BOT_VIS_TEXTURE_H_
