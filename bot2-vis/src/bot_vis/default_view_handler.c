...
static void look_at_to_matrix(const double eye[3], const double lookat[3],
                              const double _up[3], double M[16]) {
  double up[3];
...
static void build_pan_jacobian(BotDefaultViewHandler* dvh, double dq[3],
                               double up[3], double left[3], double A[4]) {
  //////////////////////////////////////////
  ...
  double PM_dq[3];
  gluProject(dq[0], dq[1], dq[2], dvh->model_matrix, dvh->projection_matrix,
             dvh->viewport, &PM_dq[0], &PM_dq[1], &PM_dq[2]);
  ...
  double PM_dq_left[3];
  gluProject(dq[0] + left[0] * eps, dq[1] + left[1] * eps,
             dq[2] + left[2] * eps, dvh->model_matrix, dvh->projection_matrix,
             dvh->viewport, &PM_dq_left[0], &PM_dq_left[1], &PM_dq_left[2]);
 ...
 */
static void window_space_pan(BotDefaultViewHandler* dvh, double dq[], double x,
                             double y, int preserveZ) {
  ...
  double new_magnitude =
      fmax(fmin(magnitude, MAX_MOTION_MAGNITUDE), MIN_MOTION_MAGNITUDE);
  bot_vector_scale_3d(motion,
                      new_magnitude / fmax(magnitude, MIN_MOTION_MAGNITUDE));
...
static int mouse_scroll(BotViewer* viewer, BotEventHandler* ehandler,
  ...
  double dq[] = {ray_start[0] - ray_dir[0] * dist,
                 ...
                 ray_start[2] - ray_dir[2] * dist, 1};

  double PM_dq[3];
  gluProject(dq[0], dq[1], dq[2], dvh->model_matrix, dvh->projection_matrix,
             dvh->viewport, &PM_dq[0], &PM_dq[1], &PM_dq[2]);

  window_space_pan(dvh, dq, event->x - PM_dq[0],
                   -((dvh->viewport[3] - event->y) - PM_dq[1]), 1);
...
static void set_camera_perspective(BotViewHandler* vhandler,
                                   double fov_degrees) {
...
static void update_gl_matrices(BotViewer* viewer, BotViewHandler* vhandler) {
  ...
  if (dvh->projection_type == BOT_VIEW_ORTHOGRAPHIC) {
    ...
    bot_vector_subtract_3d(dvh->eye, dvh->lookat, le);
    double dist =
        bot_vector_magnitude_3d(le) * tan(dvh->fov_degrees * M_PI / 180 / 2);
    glOrtho(-dist * dvh->aspect_ratio, dist * dvh->aspect_ratio, -dist, dist,
            -10 * dist, EYE_MAX_DIST * 2);
  } else {
...
static void get_eye_look(BotViewHandler* vhandler, double eye[3],
                         double look[3], double up[3]) {
  BotDefaultViewHandler* dvh = (BotDefaultViewHandler*)vhandler->user;
...
static void set_look_at(BotViewHandler* vhandler, const double eye[3],
...
static gboolean set_look_at_smooth_func(BotViewHandler* vhandler) {
  BotDefaultViewHandler* dvh = (BotDefaultViewHandler*)vhandler->user;
  ...
  for (int i = 0; i < 3; i++) {
    dvh->eye[i] = dvh->origin_eye[i] +
        time_param * (dvh->goal_eye[i] - dvh->origin_eye[i]);
    dvh->lookat[i] = dvh->origin_lookat[i] +
        time_param * (dvh->goal_lookat[i] - dvh->origin_lookat[i]);
    dvh->up[i] =
        dvh->origin_up[i] + time_param * (dvh->goal_up[i] - dvh->origin_up[i]);
  }
...
static void set_look_at_smooth(BotViewHandler* vhandler, const double eye[3],
                               const double lookat[3], const double up[3],
                               double duration_ms) {
  BotDefaultViewHandler* dvh = (BotDefaultViewHandler*)vhandler->user;
  ...
  vhandler->get_eye_look(vhandler, dvh->origin_eye, dvh->origin_lookat,
                         dvh->origin_up);
...
static void update_follow_target(BotViewHandler* vhandler, const double pos[3],
                                 const double quat[4]) {
  BotDefaultViewHandler* dvh = (BotDefaultViewHandler*)vhandler->user;
