...
void bot_gl_view_unref(BotGlView* self) { free(self); }

static void _gluPerspective(double fovy, double aspect, double znear,
...
static void _glOrtho(double left, double right, double bottom, double top,
...
static void _lookAt(const double eye_[3], const double c_[3],
                    const double up_[3], double M[16]) {
  double eye[3];
  memcpy(eye, eye_, sizeof(eye));
  double c[3];
  memcpy(c, c_, sizeof(c));
  double up[3];
  memcpy(up, up_, sizeof(up));
  ...
  double A[16] = {s[0],  s[1],  s[2],  0, u[0], u[1], u[2], 0,
                  -f[0], -f[1], -f[2], 0, 0,    0,    0,    1};

  double T[16] = {1, 0, 0, -eye[0], 0, 1, 0, -eye[1],
                  0, 0, 1, -eye[2], 0, 0, 0, 1};

  bot_matrix_multiply_4x4_4x4(A, T, M);
}

static void _recompute(BotGlView* self) {
  ...
  double pM[16];
  _gluPerspective(self->perspective_vertical_fov_degrees, aspect, 0.1, 100000,
                  pM);
...
void bot_gl_view_look_at(BotGlView* self, const double eye[3],
...
void bot_gl_view_follow(BotGlView* self, double lastPos[3], double lastQuat[4],
  ...
  if (followYaw) {
    ...
    double v2eye[3] = {lastPos[0] - self->eye[0], lastPos[1] - self->eye[1],
