...
static inline int _set_color(double rgba[4], double r, double g, double b,
  ...
  if (r < 0 || g < 0 || b < 0 || r > 1 || g > 1 || b > 1 || a < 0 || a > 1) {
    g_warning(
        "BotGlScrollPlot2d refusing to set invalid color "
        "%f, %f, %f, %f\n",
        r, g, b, a);
    return -1;
  }
...
static void __get_all_vals_helper(gpointer key, gpointer value,
                                  gpointer user_data) {
  GPtrArray* vals = (GPtrArray*)user_data;
  g_ptr_array_add(vals, value);
}

static GPtrArray* __hash_table_get_vals(GHashTable* hash_table) {
...
static void _plot2d_free(_plot2d_t* self) {
  if (self->points) {
...
BotGlScrollPlot2d* bot_gl_scrollplot2d_new() {
  ...
  self->plots = g_hash_table_new_full(g_str_hash, g_str_equal, NULL,
                                      (GDestroyNotify)_plot2d_free);
  return self;
}
...
int bot_gl_scrollplot2d_set_text_color(BotGlScrollPlot2d* self, double r,
                                       double g, double b, double a) {
  return _set_color(self->text_rgba, r, g, b, a);
}

int bot_gl_scrollplot2d_set_bgcolor(BotGlScrollPlot2d* self, double r, double g,
                                    double b, double a) {
  return _set_color(self->bg_rgba, r, g, b, a);
}

int bot_gl_scrollplot2d_set_border_color(BotGlScrollPlot2d* self, double r,
                                         double g, double b, double a) {
...
static void _plot2d_setup_gl_line_style(_plot2d_t* plot) {
  glColor4f(plot->rgba[0], plot->rgba[1], plot->rgba[2], plot->rgba[3]);
}

static void _plot2d_render_window(void* key, void* value, void* user_data) {
  _plot2d_t* plot = (_plot2d_t*)value;
  BotGlScrollPlot2d* sp = (BotGlScrollPlot2d*)user_data;

  // remove all points that aren't visible
  while (!g_queue_is_empty(plot->points)) {
    _plot2d_point_t* pt = (_plot2d_point_t*)g_queue_peek_head(plot->points);
    if (pt->x < sp->x_min) {
...
static void _draw_text(BotGlScrollPlot2d* self, const char* text) {
  for (int i = 0; i < strlen(text); i++) {
    glutBitmapCharacter(self->text_font, text[i]);
  }
}

static void draw_title(BotGlScrollPlot2d* self, int x, int y, int width,
                       int height) {
  if (!self->title || !self->show_title) {
    return;
  }

  _gl_setcolor(self->text_rgba);
  int title_width_pixels =
      glutBitmapLength(self->text_font, (unsigned char*)self->title);
  glRasterPos2f(x + width / 2 - title_width_pixels / 2,
                y + self->text_height - 1);
  _draw_text(self, self->title);
}

static void draw_axis_labels(BotGlScrollPlot2d* self, int x, int y, int width,
                             int height) {
  if (!self->show_ylim) {
    return;
  }
...
static void draw_legend(BotGlScrollPlot2d* self, int x, int y, int width,
                        int height) {
  if (self->show_legend == BOT_GL_SCROLLPLOT2D_HIDDEN) {
    return;
  }
  ...
  for (int i = 0; i < all_plots->len; i++) {
    ...
    int label_width =
        glutBitmapLength(self->text_font, (unsigned char*)plot->name);
  ...
  if (legend_x < x || legend_width > width) {
    goto done;
  }
  if (legend_y < y) {
    legend_y = y;
...
void bot_gl_scrollplot2d_gl_render_at_window_pos(BotGlScrollPlot2d* self, int x,
  ...
  // draw border
  glColor4f(self->border_rgba[0], self->border_rgba[1], self->border_rgba[2],
            self->border_rgba[3]);
  glBegin(GL_LINE_LOOP);
  glVertex2f(0, 0);
  glVertex2f(1, 0);
