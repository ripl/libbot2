...
class Tokenizer(object):
    ...
    def next_token(self):
        ...
        raise ParseError(self.line_num, self.line_pos, self.line_buf, None,
                         "Invalid character")
...
class GroupNode(object):
    ...
    def get_subgroup(self, name_parts, create=False):
        ...
        if next_name in self.subgroups:
            return self.subgroups[next_name].get_subgroup(
                name_parts[1:], create)
        elif create:
    ...
    def to_config_string(self, indent=0):
        ...
        if self.name == "":
            assert indent == 0
            val = "\n".join([
                group.to_config_string(0) for group in self.subgroups.values()
            ])
            val = val + "\n".join(
                [cmd.to_config_string(0) for cmd in self.commands]) + "\n"
        else:
            val = "%sgroup \"%s\" {\n" % (s, self.name)
            val = val + "\n".join([
...
class ConfigNode(object):
    ...
    def __str__(self):
        ...
        scripts = sorted(list(self.scripts.values()),
...
class Parser:
    ...
    def _fail(self, msg):
        raise ParseError(self.tokenizer.line_num, self.tokenizer.prev_tok_pos,
                         self.tokenizer.line_buf, self._cur_tok.val, msg)

    def _fail_next_token(self, msg):
        raise ParseError(self.tokenizer.line_num, self.tokenizer.tok_pos,
                         self.tokenizer.line_buf, self._next_tok.val, msg)
    ...
    def _parse_command_param_list(self, cmd):
        ...
        if attrib_name == "stop_signal" and attrib_val < 1:
            self._fail(
                "Invalid value specified for command attribute 'stop_signal'")
        elif attrib_name == "stop_time_allowed" and attrib_val < 1:
            self._fail("Invalid value specified for command attribute "
                       "'stop_time_allowed'")
    ...
    def _parse_run_script(self):
        script_name = self._eat_token_or_fail(TokString,
    ...
    def _parse_listdecl(self):
        while True:
            if self._eat_token(TokEOF):
                return
            ident_type = self._parse_identifier_one_of(
                ["cmd", "group", "script"])
            if ident_type == "cmd":
