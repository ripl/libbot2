...
COL_CMDS_TV_OBJ, COL_CMDS_TV_EXEC, COL_CMDS_TV_FULL_GROUP, \
    COL_CMDS_TV_COMMAND_ID, COL_CMDS_TV_HOST, COL_CMDS_TV_STATUS_ACTUAL, \
    COL_CMDS_TV_CPU_USAGE, COL_CMDS_TV_MEM_VSIZE, COL_CMDS_TV_AUTO_RESPAWN, \
...
class SheriffCommandModel(Gtk.TreeStore):
    ...
    def _find_or_make_group_row_reference(self, group_name):
        ...
        if group_name in self.group_row_references:
            ...
            if len(name_parts) > 1:
                ...
                parent_row_ref = self._find_or_make_group_row_reference(
                    parent_name)
    ...
    def _update_cmd_row(self, model_rr, cmd_deps, to_reparent):
        ...
        self.set(model_iter, COL_CMDS_TV_EXEC, cmd.exec_str,
                 COL_CMDS_TV_COMMAND_ID, command_id, COL_CMDS_TV_STATUS_ACTUAL,
                 cmd.status(), COL_CMDS_TV_HOST, cmd_deps[cmd].name,
                 COL_CMDS_TV_CPU_USAGE, cpu_str, COL_CMDS_TV_MEM_VSIZE,
                 mem_usage, COL_CMDS_TV_AUTO_RESPAWN, cmd.auto_respawn)
    ...
    def _update_group_row(self, group_rr, cmd_deps):
        ...
        child_deps = set(
            [cmd_deps[child] for child in children if child in cmd_deps])
        if len(child_deps) == 1:
        ...
        mem_total = sum([cmd.mem_vsize_bytes / 1024 for cmd in children])
        ...
        self.set(model_iter, COL_CMDS_TV_STATUS_ACTUAL, status_str,
                 COL_CMDS_TV_EXEC, exec_val, COL_CMDS_TV_HOST, dep_str,
                 COL_CMDS_TV_CPU_USAGE, cpu_str, COL_CMDS_TV_MEM_VSIZE,
                 mem_total)

    def _dispatch_row_changes(self, model, path, model_iter, user_data):
        cmds_to_add, cmd_rows_to_remove, cmds_rows_to_update, \
            group_rows_to_update = user_data
        cmd = self.iter_to_command(model_iter)
        if cmd:
    ...
    def repopulate(self):
        ...
        # On return, the cmds_to_add set will contain commands that were
        # not updated (i.e., commands that need to be added into the
        # model)
    ...
    def rows_to_commands(self, rows):
        ...
        for path in rows:
            ...
            if cmd:
                ...
                for child in self.get_group_row_child_commands_recursive(
                        cmds_iter):
                    selected.add(child)
        return selected
    ...
    def get_group_row_child_commands_recursive(self, group_iter):
        ...
        while child_iter:
            ...
            if child_cmd:
                ...
                children += self.get_group_row_child_commands_recursive(
                    child_iter)
