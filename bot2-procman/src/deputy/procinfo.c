...
#ifdef __linux__
static int procinfo_read_proc_cpu_mem_linux(int pid, proc_cpu_mem_t* s) {
...
static int procinfo_read_sys_cpu_mem_linux(sys_cpu_mem_t* s) {
  ...
  while (!feof(fp)) {
    ...
    if (!strncmp(buf, "cpu ", 4)) {
      sscanf(buf, "%s %u %u %u %u", tmp, &s->user, &s->user_low, &s->system,
             &s->idle);
      break;
    }
  }
...
int procinfo_is_orphaned_child_of(int orphan, int parent) {
  ...
  int result =
      (pinfo->ppid == 1 && pinfo->pgrp == parent && pinfo->session == parent);
  pid_info_destroy(pinfo);
  return result;
}

static GHashTable* get_all_pids_and_ppids() {
  ...
  for (const char* entry = g_dir_read_name(dir); entry;
...
int procinfo_is_orphaned_child_of(int orphan, int parent) { return 0; }
#endif
