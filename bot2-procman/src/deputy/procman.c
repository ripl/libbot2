...
static procman_cmd_t* procman_cmd_create(const char* cmd, const char* cmd_name,
                                         int32_t cmd_id);
...
procman_t* procman_create(const procman_params_t* params) {
  ...
  pm->variables = g_hash_table_new_full(
...
int procman_kill_cmd(procman_t* pm, procman_cmd_t* p, int signum) {
  if (0 == p->pid) {
...
int procman_check_for_dead_children(procman_t* pm, procman_cmd_t** dead_child) {
  int status;
  ...
  *dead_child = NULL;
  ...
  for (iter = pm->commands; iter != NULL; iter = iter->next) {
    ...
    *dead_child = p;
    ...
    for (int orphan_index = 0; orphan_index < p->descendants_to_kill->len;
         orphan_index++) {
...
int procman_close_dead_pipes(procman_t* pm, procman_cmd_t* cmd) {
  ...
  if (cmd->pid) {
    dbgt(
        "refusing to close pipes for command "
 ...
 */
static char** strsplit_set_packed(const char* tosplit, const char* delimiters,
                                  int max_tokens) {
...
static char subst_vars_has_token(subst_parse_context_t* ctx) {
  return (ctx->pos < ctx->w_len);
}

static char subst_vars_peek_token(subst_parse_context_t* ctx) {
  return subst_vars_has_token(ctx) ? ctx->w[ctx->pos] : 0;
}

static int subst_vars_eat_token(subst_parse_context_t* ctx) {
...
static int subst_vars_parse_variable(subst_parse_context_t* ctx) {
  int start = ctx->pos;
  if (!subst_vars_has_token(ctx)) {
 ...
 */
static char* subst_vars(const char* w, GHashTable* vars) {
...
static void procman_cmd_split_str(procman_cmd_t* pcmd, GHashTable* variables) {
  ...
  int argc = -1;
  GError* err = NULL;
  gboolean parsed = g_shell_parse_argv(pcmd->cmd->str, &argc, &argv, &err);
...
const GList* procman_get_cmds(procman_t* pm) { return pm->commands; }
...
int procman_remove_cmd(procman_t* pm, procman_cmd_t* cmd) {
  ...
  if (!toremove) {
    dbgt(
        "procman ERRROR: %s does not appear to be managed "
        "by this procman!!\n",
        cmd->cmd->str);
    return -1;
  }
