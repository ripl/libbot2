...
static void dbgt(const char* fmt, ...) {
  ...
  pos += snprintf(timebuf + pos, sizeof(timebuf) - pos, ".%03d",
...
typedef struct _procman_deputy {
  ...
  int nstale_orders_slm;  // total stale bot_procman_orders2_t for this deputy
                          // slm
...
static void transmit_proc_info(procman_deputy_t* s);

static gboolean on_scheduled_respawn(procman_cmd_t* cmd);

static void transmit_str(procman_deputy_t* pmd, int sid, char* str) {
...
static void printf_and_transmit(procman_deputy_t* pmd, int sid, char* fmt,
                                ...) {
  int len;
  char buf[256];
  va_list ap;
...
static int pipe_data_ready(GIOChannel* source, GIOCondition condition,
                           procman_cmd_t* cmd) {
  ...
  if (!anycondition) {
    dbgt("wtf??? [%s] pipe has condition 0x%X\n", cmd->cmd->str, condition);
  }
  return result;
}

static void maybe_schedule_respawn(procman_deputy_t* pmd, procman_cmd_t* cmd) {
  pmd_cmd_moreinfo_t* mi = (pmd_cmd_moreinfo_t*)cmd->user;
  if (mi->auto_respawn && !mi->should_be_stopped && !pmd->exiting) {
    mi->respawn_timeout_id = g_timeout_add(
        mi->respawn_backoff, (GSourceFunc)on_scheduled_respawn, cmd);
  }
}

static int start_cmd(procman_deputy_t* pmd, procman_cmd_t* cmd,
                     int desired_runid) {
  if (pmd->exiting) {
    return -1;
  }
  ...
  if (ms_since_started < MAX_RESPAWN_DELAY_MS) {
    mi->respawn_backoff =
        MIN(MAX_RESPAWN_DELAY_MS, mi->respawn_backoff * RESPAWN_BACKOFF_RATE);
  } else {
    int d = ms_since_started / MAX_RESPAWN_DELAY_MS;
    mi->respawn_backoff = MAX(MIN_RESPAWN_DELAY_MS, mi->respawn_backoff >> d);
  }
  ...
  mi->stdout_sid =
      g_io_add_watch(mi->stdout_ioc, G_IO_IN, (GIOFunc)pipe_data_ready, cmd);
...
static int stop_cmd(procman_deputy_t* pmd, procman_cmd_t* cmd) {
  if (!cmd->pid) {
    return 0;
  }
  ...
  if (0 != status) {
    printf_and_transmit(pmd, mi->sheriff_id, "kill: %s\n", strerror(-status));
  }
  return status;
}

static void check_for_dead_children(procman_deputy_t* pmd) {
  ...
  while (cmd) {
    ...
    if (WIFSIGNALED(cmd->exit_status)) {
      ...
      printf_and_transmit(pmd, mi->sheriff_id, "%s\n", strsignal(signum),
                          signum);
...
static gboolean on_quit_timeout(procman_deputy_t* pmd) {
...
static void glib_handle_signal(int signal, procman_deputy_t* pmd) {
  if (signal == SIGCHLD) {
...
static void transmit_proc_info(procman_deputy_t* s) {
  if (s->messaging_version == 1) {
    ...
    msg.cmds = (bot_procman_deputy_cmd_t*)calloc(
        msg.ncmds, sizeof(bot_procman_deputy_cmd_t));
    ...
    msg.ncmds = g_list_length((GList*)allcmds);
    msg.cmds = (bot_procman_deputy_cmd2_t*)calloc(
        msg.ncmds, sizeof(bot_procman_deputy_cmd2_t));
...
static void update_cpu_times(procman_deputy_t* s) {
  const GList* allcmds = procman_get_cmds(s->pm);
  const GList* iter;
  int status;
...
static gboolean one_second_timeout(procman_deputy_t* pmd) {
  update_cpu_times(pmd);
  transmit_proc_info(pmd);
  return TRUE;
}

static gboolean introspection_timeout(procman_deputy_t* s) {
  int mypid = getpid();
  ...
  dbgt("MARK - rss: %" PRId64 " kB vsz: %" PRId64 " kB procs: %d (%d alive)\n",
       pinfo.rss / 1024, pinfo.vsize / 1024,
       g_list_length((GList*)procman_get_cmds(s->pm)), nrunning);

  s->norders_slm = 0;
  s->norders_forme_slm = 0;
  s->nstale_orders_slm = 0;

  for (GList* ositer = s->observed_sheriffs_slm; ositer;
       ositer = ositer->next) {
    free(ositer->data);
  }
...
static bot_procman_sheriff_cmd2_t* procmd_orders_find_cmd(
    const bot_procman_orders2_t* a, int32_t sheriff_id) {
  int i;
  for (i = 0; i < a->ncmds; i++) {
    if (sheriff_id == a->cmds[i].sheriff_id) {
      return &a->cmds[i];
    }
  }
  return NULL;
...
static procman_cmd_t* find_local_cmd(procman_deputy_t* s, int32_t sheriff_id) {
  const GList* iter;
...
static void _set_command_group(procman_cmd_t* p, const char* group) {
  pmd_cmd_moreinfo_t* mi = p->user;
  free(mi->group);
  mi->group = strdup(group);
}

static void _set_command_stop_signal(procman_cmd_t* p, int stop_signal) {
  pmd_cmd_moreinfo_t* mi = p->user;
  mi->stop_signal = stop_signal;
}

static void _set_command_stop_time_allowed(procman_cmd_t* p,
                                           float stop_time_allowed) {
  pmd_cmd_moreinfo_t* mi = p->user;
  mi->stop_time_allowed = stop_time_allowed;
}

static void _set_command_nickname(procman_cmd_t* p, const char* nickname) {
  pmd_cmd_moreinfo_t* mi = p->user;
  free(mi->nickname);
  mi->nickname = strdup(nickname);
}

static void _handle_orders2(procman_deputy_t* s,
                            const bot_procman_orders2_t* orders,
                            int messaging_version) {
  ...
  if (now - orders->utime > PROCMAN_MAX_MESSAGE_AGE_USEC) {
    for (int i = 0; i < orders->ncmds; i++) {
      ...
      printf_and_transmit(s, cmd_msg->sheriff_id,
                          "ignoring stale orders (utime %d seconds ago). You "
                          "may want to check the system clocks!\n",
  ...
  for (ositer = s->observed_sheriffs_slm; ositer; ositer = ositer->next) {
    if (!strcmp((char*)ositer->data, orders->sheriff_name)) {
      break;
    }
  }
  if (!ositer) {
    s->observed_sheriffs_slm =
        g_list_prepend(s->observed_sheriffs_slm, strdup(orders->sheriff_name));
  }
  ...
  for (i = 0; i < orders->ncmds; i++) {
    ...
    if (strcmp(mi->nickname, cmd_msg->cmd.command_name)) {
      dbgt("[%s] rename -> [%s]\n", mi->nickname, cmd_msg->cmd.command_name);
    ...
    if (mi->stop_signal != cmd_msg->cmd.stop_signal) {
      dbg("[%s] stop signal -> [%d]\n", p->cmd_name, cmd_msg->cmd.stop_signal);
...
static void procman_deputy_order2_received(const lcm_recv_buf_t* rbuf,
                                           const char* channel,
                                           const bot_procman_orders2_t* orders,
                                           void* user_data) {
...
static void procman_deputy_order_received(const lcm_recv_buf_t* rbuf,
                                          const char* channel,
                                          const bot_procman_orders_t* orders,
                                          void* user_data) {
  procman_deputy_t* deputy = user_data;
  ...
  new_orders.cmds = (bot_procman_sheriff_cmd2_t*)calloc(
      orders->ncmds, sizeof(bot_procman_sheriff_cmd2_t));
  int cmd_index;
...
static void procman_deputy_discovery_received(
  ...
  if (now < pmd->deputy_start_time + DISCOVERY_TIME_MS * 1000) {
    ...
    if (!strcmp(msg->host, pmd->hostname) && msg->nonce != pmd->deputy_pid) {
      dbgt(
          "ERROR.  Detected another deputy named [%s].  Aborting to avoid "
          "conflicts.\n",
          msg->host);
      exit(1);
    }
...
static void procman_deputy_info2_received(const lcm_recv_buf_t* rbuf,
                                          const char* channel,
                                          const bot_procman_info2_t* msg,
                                          void* user_data) {
  ...
  if (now < pmd->deputy_start_time + DISCOVERY_TIME_MS * 1000) {
    ...
    dbgt(
        "WARNING:  Still processing info messages while not in discovery "
        "mode??\n");
  }
}

static void procman_deputy_info_received(const lcm_recv_buf_t* rbuf,
                                         const char* channel,
                                         const bot_procman_info_t* msg,
                                         void* user_data) {
  procman_deputy_t* pmd = user_data;
...
static gboolean discovery_timeout(procman_deputy_t* pmd) {
  int64_t now = timestamp_now();
  if (now < pmd->deputy_start_time + DISCOVERY_TIME_MS * 1000) {
  ...
  pmd->discovery_subs = bot_procman_discovery_t_subscribe(
      pmd->lcm, "PMD_DISCOVER", procman_deputy_discovery_received, pmd);

  pmd->orders_subs = bot_procman_orders_t_subscribe(
      pmd->lcm, "PMD_ORDERS", procman_deputy_order_received, pmd);
  pmd->orders2_subs = bot_procman_orders2_t_subscribe(
      pmd->lcm, "PMD_ORDERS2", procman_deputy_order2_received, pmd);
...
static void usage() {
  fprintf(
      stderr,
      "usage: bot-procman-deputy [options]\n"
      "\n"
...
int main(int argc, char** argv) {
  ...
  signal_pipe_attach_glib((signal_pipe_glib_handler_t)glib_handle_signal, pmd);

  // setup LCM handler
  lcmu_glib_mainloop_attach_lcm(pmd->lcm);

  pmd->info_subs = bot_procman_info_t_subscribe(
      pmd->lcm, "PMD_INFO", procman_deputy_info_received, pmd);
  pmd->info2_subs = bot_procman_info2_t_subscribe(
      pmd->lcm, "PMD_INFO2", procman_deputy_info2_received, pmd);
